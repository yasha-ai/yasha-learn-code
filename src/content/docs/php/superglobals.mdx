---
title: "10. Суперглобальные переменные"
sidebar:
  order: 9
---
...
<html>
<head>
  <title>Суперглобальные переменные</title>
  <style>
...
<body>
  <h1>Привет!</h1>
  <p>Нажми кнопку, чтобы увидеть IP адрес (имитация \$_SERVER['REMOTE_ADDR']):</p>
  <button id="myButton">Показать IP</button>
  <p id="ipAddress"></p>

  <script>
    document.getElementById('myButton').addEventListener('click', function() {
      // В реальном PHP это было бы \$_SERVER['REMOTE_ADDR']
      const fakeIp = '192.168.1.1';
      document.getElementById('ipAddress').textContent = 'Ваш IP: ' + fakeIp;
    });
  <\/script>
</body>
</html>
