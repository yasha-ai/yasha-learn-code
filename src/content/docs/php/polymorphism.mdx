...
<body>
  <h1>Полиморфизм в PHP (Эмуляция)</h1>
  <button id="myButton">Выполнить действие</button>
  <div id="output"></div>
  ...
  <script>
    ...
    button.addEventListener('click', () => {
      // Эмуляция полиморфизма через JavaScript
      function doSomething(obj) {
        output.textContent = obj.action();
      }

      const object1 = { action: () => "Действие 1" };
      const object2 = { action: () => "Действие 2" };

      // Вызываем функцию с разными объектами
      doSomething(object1);
      setTimeout(() => doSomething(object2), 2000); // Задержка для демонстрации
    });
  <\/script>
</body>
</html>
