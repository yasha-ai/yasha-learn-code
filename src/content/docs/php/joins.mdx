...
<body>
  <h1>JOIN Demo</h1>
  <button id="joinButton">Simulate JOIN</button>
  <div id="output"></div>

  <script>
    const users = [{id: 1, name: 'Alice', city_id: 1}, {id: 2, name: 'Bob', city_id: 2}];
    const cities = [{id: 1, name: 'New York'}, {id: 2, name: 'London'}];

    document.getElementById('joinButton').addEventListener('click', () => {
      const joinedData = users.map(user => {
        const city = cities.find(city => city.id === user.city_id);
        return \`User: \${user.name}, City: \${city ? city.name : 'Unknown'}\`;
      });

      document.getElementById('output').innerHTML = joinedData.join('<br>');
    });
  <\/script>
</body>
</html>
