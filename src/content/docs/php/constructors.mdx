...
<body>
  <div class="container">
    <h1>PHP Конструкторы</h1>
    <p>Нажмите кнопку, чтобы создать объект с использованием конструктора.</p>
    <button id="createObject">Создать Объект</button>
    <div id="output"></div>
  </div>

  <script>
    document.getElementById('createObject').addEventListener('click', function() {
      fetch('/constructor.php')
...
class MyClass {
  ...
  public function __construct(\$msg = "Привет из конструктора!") {
    \$this->message = \$msg;
  }
...
\$obj2 = new MyClass("Новое сообщение!");
echo "<p>" . \$obj2->getMessage() . "</p>";
?>
`
  }}
/>
