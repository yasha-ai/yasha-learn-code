...
### Playground: Singleton в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// Singleton: только один экземпляр класса
...
class ConfigManager {
  ...
  static getInstance() {
    if (!ConfigManager.instance) {
      ConfigManager.instance = new ConfigManager();
      console.log("ConfigManager created (once!)");
    }
    return ConfigManager.instance;
  }

  constructor() {
    this.config = { apiUrl: "https://api.example.com", debug: false };
  }
...
c1.set("debug", true);
console.log("c2.debug:", c2.get("debug")); // true (тот же объект)

// Module-level singleton (Node.js style)
const logger = (() => {
  const logs = [];
  return {
    log(msg) { logs.push(msg); console.log("[LOG]", msg); },
    getLogs() { return logs; }
  };
})();
