...
### Playground: Strategy в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// Strategy: взаимозаменяемые алгоритмы
...
// Контекст переключает стратегию
class Sorter {
  ...
  sort(data) {
    ...
    console.log("Sorted in " + ms + "ms:", result.slice(0, 8).join(", ") + "...");
    return result;
  }
}
