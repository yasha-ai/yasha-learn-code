...
### Playground: Facade в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// Facade: упрощённый интерфейс к сложной подсистеме

// Подсистема 1: валидация
class Validator {
  isValidEmail(email) { return email.includes("@"); }
  isValidName(name) { return name.length >= 2; }
}

// Подсистема 2: хранилище
class UserStore {
  constructor() { this.users = []; }
  save(user) { this.users.push(user); return { ...user, id: this.users.length }; }
  findByEmail(email) { return this.users.find(u => u.email === email); }
}

// Подсистема 3: уведомления
class Notifier {
  send(email, msg) { console.log("[Notify " + email + "]", msg); }
}

// FACADE: скрывает всю сложность за простым API
class UserFacade {
  constructor() {
  ...
  register(name, email) {
    if (!this.validator.isValidName(name)) throw new Error("Bad name");
    if (!this.validator.isValidEmail(email)) throw new Error("Bad email");
    if (this.store.findByEmail(email)) throw new Error("Already exists");

    const user = this.store.save({ name, email });
    this.notifier.send(email, "Welcome, " + name + "!");
    return user;
  }
}

// Клиент вызывает один метод вместо 5 шагов
const facade = new UserFacade();
console.log(facade.register("Alice", "alice@mail.com"));
console.log(facade.register("Bob", "bob@mail.com"));
try { facade.register("Alice", "alice@mail.com"); }
