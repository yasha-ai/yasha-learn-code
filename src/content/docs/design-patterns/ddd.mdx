...
### Playground: DDD Value Object + Entity

<Playground client:visible template="vanilla" files={{ '/index.js': `// DDD: Value Object (Money) + Entity (Order) + Aggregate
...
class Order {
  ...
  addItem(name, price, qty) {
    ...
    this.items.push(new OrderItem(name, price, qty));
  }
  total() {
    return this.items.reduce((sum, i) => sum.add(i.subtotal()), new Money(0, "USD"));
  }
  confirm() {
    if (!this.items.length) throw new Error("Cannot confirm empty order");
    this.status = "CONFIRMED";
    console.log("Order " + this.id + " confirmed! Total: " + this.total());
  }
}

const order = new Order("ORD-1");
order.addItem("Laptop", new Money(999, "USD"), 1);
order.addItem("Mouse", new Money(29, "USD"), 2);
console.log("Total:", order.total().toString());
order.confirm();

try { order.addItem("Keyboard", new Money(79, "USD"), 1); }
