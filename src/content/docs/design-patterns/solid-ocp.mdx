...
### Playground: OCP в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// OCP: открыт для расширения, закрыт для изменений
...
class DiscountCalculator {
  ...
  calculate(total, type) {
    ...
    return fn ? fn(total) : 0;
  }
}

// Добавляем стратегии БЕЗ изменения DiscountCalculator
const calc = new DiscountCalculator();
calc.register("seasonal", (t) => t * 0.1);
calc.register("loyalty", (t) => t * 0.15);
calc.register("black_friday", (t) => t * 0.3);

// Легко добавить новую — ничего не ломается
calc.register("student", (t) => t * 0.25);

const total = 1000;
console.log("Seasonal:", calc.calculate(total, "seasonal"));
console.log("Loyalty:", calc.calculate(total, "loyalty"));
console.log("Black Friday:", calc.calculate(total, "black_friday"));
console.log("Student:", calc.calculate(total, "student"));
console.log("Unknown:", calc.calculate(total, "vip"));
` }} />
