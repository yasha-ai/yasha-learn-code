---
title: "16. Microservices Architecture"
sidebar:
  order: 16
---
...
<Playground client:visible template="vanilla" files={{ '/index.js': `// Circuit Breaker: защита от каскадных сбоев

class CircuitBreaker {
  constructor(threshold = 3, timeout = 2000) {
    this.state = "CLOSED";
    this.failures = 0;
  ...
  async call(fn) {
    if (this.state === "OPEN") {
      if (Date.now() - this.lastFailure > this.timeout) {
        this.state = "HALF-OPEN";
        console.log("  [CB] -> HALF-OPEN (trying again)");
      } else {
    ...
    try {
      ...
      if (this.failures >= this.threshold || this.state === "HALF-OPEN") {
        this.state = "OPEN";
        console.log("  [CB] -> OPEN (threshold reached)");
      }
      throw err;
    }
  }
...
async function demo() {
  ...
  for (let i = 1; i <= 7; i++) {
    try {
      const res = await cb.call(unstableService);
      console.log("Call " + i + ": " + res + " [" + cb.state + "]");
    } catch (e) {
      console.log("Call " + i + ": FAIL - " + e.message + " [" + cb.state + "]");
    }
