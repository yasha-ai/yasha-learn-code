...
### Playground: Adapter в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// Adapter: несовместимые интерфейсы -> единый API

// Старый API (callback-based)
class LegacyWeatherService {
  fetchWeather(city, callback) {
    setTimeout(() => callback(null, { temp: 22, wind: 5 }), 100);
  }
}

// Новый API (Promise-based) — адаптер оборачивает старый
class WeatherAdapter {
  constructor(legacy) { this.legacy = legacy; }
  getWeather(city) {
    return new Promise((resolve, reject) => {
      this.legacy.fetchWeather(city, (err, data) => {
        if (err) reject(err);
        else resolve({ city, temperature: data.temp + "C", wind: data.wind + " m/s" });
      });
    });
  }
}

// Ещё один адаптер: XML-like объект -> единый формат
class XmlWeatherAdapter {
  constructor() {}
  getWeather(city) {
...
async function showWeather(adapter, city) {
  const data = await adapter.getWeather(city);
  console.log(city + ": " + data.temperature + ", wind " + data.wind);
}

const legacy = new WeatherAdapter(new LegacyWeatherService());
const xml = new XmlWeatherAdapter();

showWeather(legacy, "Moscow");
showWeather(xml, "Berlin");
` }} />
