...
### Playground: Builder в действии

<Playground client:visible template="vanilla" files={{ '/index.js': `// Builder: пошаговое создание сложных объектов

class QueryBuilder {
  constructor() {
    this.query = { table: "", cols: ["*"], conditions: [], order: null, lim: null };
  }
  from(table) { this.query.table = table; return this; }
  select(...cols) { this.query.cols = cols; return this; }
  where(cond) { this.query.conditions.push(cond); return this; }
  orderBy(col, dir = "ASC") { this.query.order = col + " " + dir; return this; }
  limit(n) { this.query.lim = n; return this; }

  build() {
    const parts = ["SELECT " + this.query.cols.join(", ") + " FROM " + this.query.table];
    if (this.query.conditions.length) parts.push("WHERE " + this.query.conditions.join(" AND "));
    if (this.query.order) parts.push("ORDER BY " + this.query.order);
    if (this.query.lim) parts.push("LIMIT " + this.query.lim);
    return parts.join("\\n");
  }
}

// Fluent API — читаемо и понятно
