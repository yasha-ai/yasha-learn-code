...
<body>
  ...
  <script>
    ...
    function draw() {
      ...
      // Отскок от стен
      if (ballX + bdx > canvas.width - radius || ballX + bdx < radius) bdx = -bdx;
      if (ballY + bdy > canvas.height - radius || ballY + bdy < radius) bdy = -bdy;
