...
![Иллюстрация к уроку](/lessons/react-use-ref-reducer.png)
...
<Playground client:visible
  template="react"
  files={{
    "/App.js": `import { useReducer, useRef } from 'react';
...
export default function App() {
  ...
  const inputRef = useRef(null);
  const [log, logDispatch] = useReducer((s, a) => [a, ...s.slice(0, 4)], []);

  const send = (type, payload) => {
    dispatch({ type, payload });
    logDispatch(payload !== undefined ? type + '(' + payload + ')' : type);
  };

  const handleSet = () => {
    const val = parseInt(inputRef.current.value);
    if (!isNaN(val)) send('SET', val);
  };

  return (
    <div style={{ padding: 20, background: '#1e1e2e', color: '#cdd6f4', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <h2 style={{ margin: '0 0 16px' }}>useReducer + useRef</h2>
      <div style={{ background: '#313244', borderRadius: 8, padding: 20, marginBottom: 12 }}>
        <div style={{ fontSize: 13, color: '#bac2de', marginBottom: 8 }}>useReducer — управляет счётчиком:</div>
        <div style={{ fontSize: 48, fontWeight: 'bold', color: '#89b4fa', textAlign: 'center', margin: '12px 0' }}>{state.count}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 4 }}>
          <button onClick={() => send('DEC')} style={btn()}>−</button>
          <button onClick={() => send('INC')} style={btn()}>+</button>
          <button onClick={() => send('RESET')} style={btn('#45475a')}>↺ Reset</button>
        </div>
      </div>
      <div style={{ background: '#313244', borderRadius: 8, padding: 16, marginBottom: 12 }}>
        <div style={{ fontSize: 13, color: '#bac2de', marginBottom: 8 }}>useRef — прямой доступ к DOM:</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <input ref={inputRef} type="number" defaultValue="42"
            style={{ background: '#181825', color: '#cdd6f4', border: '1px solid #45475a', padding: '6px 10px', borderRadius: 4, flex: 1 }} />
          <button onClick={handleSet} style={btn('#a6e3a1')}>SET</button>
          <button onClick={() => { inputRef.current.focus(); inputRef.current.select(); }} style={btn('#cba6f7')}>Focus</button>
        </div>
      </div>
      <div style={{ background: '#181825', borderRadius: 8, padding: 12 }}>
        <div style={{ fontSize: 12, color: '#bac2de', marginBottom: 4 }}>Лог dispatch:</div>
        {log.length === 0
          ? <div style={{ color: '#585b70', fontSize: 13 }}>Нет действий</div>
          : log.map((l, i) => <div key={i} style={{ color: '#a6e3a1', fontFamily: 'monospace', fontSize: 13 }}>→ dispatch({l})</div>)}
      </div>
    </div>
  );
}`,
