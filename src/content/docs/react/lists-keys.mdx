...
export default function App() {
  ...
  const addItem = () => {
    ...
    // Генерируем уникальный ID для нового элемента
    setItems([...items, { id: newId, text: \`Новый элемент \${newId.toString().slice(-3)}\` }]);
  };
  ...
  return (
    <div style={{ padding: '20px', background: '#282c34', color: 'white', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <h2>Список с ключами (Keys)</h2>
      <button onClick={addItem} style={{ padding: '8px 12px', marginBottom: '15px', cursor: 'pointer' }}>
        Добавить элемент
      </button>
      
      <ul style={{ listStyleType: 'none', padding: 0 }}>
        {items.map((item) => (
          // Уникальный key помогает React эффективно обновлять DOM при удалении/добавлении
          <li key={item.id} style={{ background: '#3a3f4b', margin: '8px 0', padding: '10px', borderRadius: '4px', display: 'flex', justifyContent: 'space-between' }}>
