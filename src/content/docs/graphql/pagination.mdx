...
const allPosts = Array.from({ length: 25 }, (_, i) => ({
  id: i + 1, title: \`Пост #\${i + 1}\`, date: \`2026-0\${Math.ceil((i+1)/5)}-\${10+i%20}\`
}));
...
export default function App() {
  ...
  const offsetData = allPosts.slice(offset, offset + limit);
  const cursorData = allPosts.slice(cursor, cursor + limit);

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>Pagination: offset vs cursor</h2>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button onClick={() => setMode('offset')} style={{ padding: '6px 12px', background: mode === 'offset' ? '#6366f1' : '#e5e7eb', color: mode === 'offset' ? '#fff' : '#333', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Offset</button>
        <button onClick={() => setMode('cursor')} style={{ padding: '6px 12px', background: mode === 'cursor' ? '#6366f1' : '#e5e7eb', color: mode === 'cursor' ? '#fff' : '#333', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Cursor</button>
      </div>
      {mode === 'offset' ? (
        <div>
          <p style={{ fontSize: 13, color: '#666' }}>offset: {offset}, limit: {limit}</p>
          <ul>{offsetData.map(p => <li key={p.id}>{p.title} ({p.date})</li>)}</ul>
          <button disabled={offset === 0} onClick={() => setOffset(o => o - limit)} style={{ marginRight: 8 }}>← Назад</button>
          <button disabled={offset + limit >= allPosts.length} onClick={() => setOffset(o => o + limit)}>Далее →</button>
        </div>
      ) : (
        <div>
          <p style={{ fontSize: 13, color: '#666' }}>after cursor: {cursor}, first: {limit}</p>
          <ul>{cursorData.map(p => <li key={p.id}>{p.title} ({p.date})</li>)}</ul>
          <button disabled={cursor + limit >= allPosts.length} onClick={() => setCursor(c => c + limit)}>Загрузить ещё →</button>
        </div>
      )}
    </div>
  );
