...
export default function App() {
  ...
  const simulate = () => {
    ...
    const posts = [
      ...
      { title: 'Post 2', authorId: 2 },
      { title: 'Post 3', authorId: 1 },
      { title: 'Post 4', authorId: 3 },
      { title: 'Post 5', authorId: 2 },
    ];

    if (mode === 'naive') {
      newLog.push('SELECT * FROM posts LIMIT 5  — 1 запрос');
      posts.forEach(p => {
        newLog.push(\`SELECT * FROM users WHERE id = \${p.authorId}  — N запросов!\`);
      });
      newLog.push(\`Итого: \${1 + posts.length} SQL запросов (N+1 проблема)\`);
    } else {
      newLog.push('SELECT * FROM posts LIMIT 5  — 1 запрос');
      const ids = [...new Set(posts.map(p => p.authorId))];
      newLog.push(\`DataLoader batch: SELECT * FROM users WHERE id IN (\${ids.join(',')})  — 1 запрос\`);
      newLog.push(\`Итого: 2 SQL запроса (DataLoader решает N+1)\`);
    }
    setLog(newLog);
  };
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>DataLoader: решение N+1</h2>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button onClick={() => setMode('naive')} style={{ padding: '6px 12px', background: mode === 'naive' ? '#ef4444' : '#e5e7eb', color: mode === 'naive' ? '#fff' : '#333', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Без DataLoader</button>
        <button onClick={() => setMode('dataloader')} style={{ padding: '6px 12px', background: mode === 'dataloader' ? '#22c55e' : '#e5e7eb', color: mode === 'dataloader' ? '#fff' : '#333', border: 'none', borderRadius: 6, cursor: 'pointer' }}>С DataLoader</button>
        <button onClick={simulate} style={{ padding: '6px 12px', cursor: 'pointer' }}>Симулировать</button>
      </div>
      <div style={{ fontFamily: 'monospace', fontSize: 13 }}>
        {log.map((line, i) => (
          <div key={i} style={{ padding: 4, color: line.includes('Итого') ? (mode === 'naive' ? '#ef4444' : '#22c55e') : '#333' }}>{line}</div>
        ))}
      </div>
    </div>
  );
