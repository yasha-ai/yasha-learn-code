---
title: "11. React Hooks для GraphQL"
sidebar:
  order: 11
---
...
// Имитация хуков Apollo Client
function useMockQuery() {
  const [state, setState] = useState({ loading: false, data: null, error: null });
  const refetch = () => {
    setState({ loading: true, data: null, error: null });
    setTimeout(() => {
      setState({ loading: false, data: { users: [{ name: 'Алексей' }, { name: 'Мария' }] }, error: null });
    }, 500);
  };
...
function useMockMutation() {
  ...
  const mutate = (vars) => {
    ...
    setTimeout(() => {
      setState({ loading: false, data: { createUser: { name: vars.name, id: Date.now() } } });
    }, 400);
  };
  return [mutate, state];
}
...
export default function App() {
  const { loading, data, refetch } = useMockQuery();
  const [createUser, mutation] = useMockMutation();
  const [name, setName] = useState('');

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>React Hooks для GraphQL</h2>
      <div style={{ marginBottom: 16 }}>
        <h3>useQuery</h3>
        <button onClick={refetch} style={{ padding: '6px 12px', cursor: 'pointer' }}>
          {loading ? 'Загрузка...' : 'refetch()'}
        </button>
        {data && <ul>{data.users.map((u, i) => <li key={i}>{u.name}</li>)}</ul>}
      </div>
      <div>
        <h3>useMutation</h3>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Имя"
          style={{ padding: 6, marginRight: 8, borderRadius: 4, border: '1px solid #ccc' }} />
        <button onClick={() => createUser({ name })} style={{ padding: '6px 12px', cursor: 'pointer' }}>
          {mutation.loading ? '...' : 'createUser()'}
        </button>
        {mutation.data && <p>Создан: {mutation.data.createUser.name}</p>}
      </div>
    </div>
  );
}
