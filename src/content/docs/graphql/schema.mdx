---
title: "3. Schema Definition Language"
sidebar:
  order: 3
---
...
// SDL визуализация — определение схемы
const schema = {
  User: { id: 'ID!', name: 'String!', email: 'String!', role: 'UserRole!', posts: '[Post!]!' },
  Post: { id: 'ID!', title: 'String!', body: 'String!', status: 'PostStatus!', author: 'User!' },
  Query: { user: '(id: ID!): User', users: '(role: UserRole): [User!]!', posts: '(limit: Int = 20): [Post!]!' },
  Mutation: { createPost: '(input: CreatePostInput!): Post!', deletePost: '(id: ID!): Boolean!' },
};
...
export default function App() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>SDL Schema Explorer</h2>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {types.map(t => (
          <button key={t} onClick={() => setSelected(t)}
            style={{
              padding: '6px 14px', cursor: 'pointer', borderRadius: 6,
              background: selected === t ? '#6366f1' : '#f3f4f6',
              color: selected === t ? '#fff' : '#333', border: 'none',
            }}>
            {t}
          </button>
        ))}
      </div>
      <pre style={{ background: '#1e1e1e', color: '#d4d4d4', padding: 16, borderRadius: 8 }}>{(selected === 'Query' || selected === 'Mutation'
  ? 'type ' + selected + ' {\\n' + Object.entries(schema[selected]).map(([k,v]) => '  ' + k + v).join('\\n') + '\\n}'
  : 'type ' + selected + ' {\\n' + Object.entries(schema[selected]).map(([k,v]) => '  ' + k + ': ' + v).join('\\n') + '\\n}'
)}</pre>
    </div>
  );
