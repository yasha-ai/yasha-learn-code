...
export default function App() {
  ...
  const addBook = () => {
    ...
    const mutation = \`mutation { createBook(input: { title: "\${title}", author: "\${author}" }) { id title } }\`;
    setLog(prev => [...prev, mutation]);
    setBooks(prev => [...prev, { id: Date.now(), title, author }]);
    setTitle(''); setAuthor('');
  };

  const deleteBook = (id) => {
    setLog(prev => [...prev, \`mutation { deleteBook(id: "\${id}") }\`]);
    setBooks(prev => prev.filter(b => b.id !== id));
  };

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>Mutations: создание и удаление</h2>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <input placeholder="Название" value={title} onChange={e => setTitle(e.target.value)}
          style={{ padding: 6, borderRadius: 4, border: '1px solid #ccc' }} />
        <input placeholder="Автор" value={author} onChange={e => setAuthor(e.target.value)}
          style={{ padding: 6, borderRadius: 4, border: '1px solid #ccc' }} />
        <button onClick={addBook} style={{ padding: '6px 12px', cursor: 'pointer' }}>Добавить</button>
      </div>
      <ul>{books.map(b => (
        <li key={b.id}>{b.title} — {b.author}
          <button onClick={() => deleteBook(b.id)} style={{ marginLeft: 8, cursor: 'pointer' }}>✕</button>
        </li>
      ))}</ul>
      {log.length > 0 && (
        <pre style={{ background: '#1e1e1e', color: '#d4d4d4', padding: 12, borderRadius: 8, fontSize: 12, maxHeight: 120, overflow: 'auto' }}>
          {log.join('\n')}
        </pre>
      )}
    </div>
