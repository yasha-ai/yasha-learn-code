...
const generated = \`// Auto-generated by GraphQL Codegen
...
export default function App() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>GraphQL Codegen: Schema → TypeScript</h2>
      <div style={{ display: 'flex', gap: 16 }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 14 }}>schema.graphql</h3>
          <pre style={{ background: '#1e1e1e', color: '#d4d4d4', padding: 12, borderRadius: 8, fontSize: 12 }}>{schema}</pre>
        </div>
        {showGenerated && (
          <div style={{ flex: 1 }}>
            <h3 style={{ fontSize: 14 }}>generated/types.ts</h3>
            <pre style={{ background: '#0d1117', color: '#7ee787', padding: 12, borderRadius: 8, fontSize: 12 }}>{generated}</pre>
          </div>
        )}
      </div>
      <button onClick={() => setShowGenerated(!showGenerated)}
        style={{ marginTop: 12, padding: '8px 16px', cursor: 'pointer', background: '#6366f1', color: '#fff', border: 'none', borderRadius: 6 }}>
        {showGenerated ? 'Скрыть' : 'npx graphql-codegen →'}
      </button>
    </div>
  );
}
