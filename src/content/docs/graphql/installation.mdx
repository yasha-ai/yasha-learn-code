...
// Имитация Apollo Client — показываем структуру useQuery
function useMockQuery(queryName) {
...
export default function App() {
  const { loading, data, execute } = useMockQuery('GET_BOOKS');

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>Apollo Client: useQuery демо</h2>
      <button onClick={execute} style={{ padding: '8px 16px', cursor: 'pointer' }}>
        Выполнить GET_BOOKS
      </button>
      {loading && !data && <p>Нажмите кнопку для загрузки...</p>}
      {data && (
        <ul>
          {data.books.map((b, i) => (
            <li key={i}><strong>{b.title}</strong> — {b.author}</li>
          ))}
        </ul>
      )}
    </div>
