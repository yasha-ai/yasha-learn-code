...
const typeCategories = {
  'Scalar': ['String', 'Int', 'Float', 'Boolean', 'ID', 'Date (custom)'],
  'Object': ['type User { id: ID!, name: String! }'],
  'Enum': ['enum Role { ADMIN, EDITOR, VIEWER }'],
  'Interface': ['interface Node { id: ID! }'],
  'Union': ['union SearchResult = User | Post | Product'],
  'Input': ['input CreateUserInput { name: String!, email: String! }'],
};

export default function App() {
  const [active, setActive] = useState('Scalar');

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>Система типов GraphQL</h2>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
        {Object.keys(typeCategories).map(cat => (
          <button key={cat} onClick={() => setActive(cat)} style={{
            padding: '6px 12px', borderRadius: 6, border: 'none', cursor: 'pointer',
            background: active === cat ? '#8b5cf6' : '#e5e7eb',
            color: active === cat ? '#fff' : '#333',
          }}>{cat}</button>
        ))}
      </div>
      <div style={{ background: '#f9fafb', padding: 16, borderRadius: 8, border: '1px solid #e5e7eb' }}>
        {typeCategories[active].map((item, i) => (
          <pre key={i} style={{ margin: '4px 0', fontFamily: 'monospace' }}>{item}</pre>
        ))}
      </div>
    </div>
  );
