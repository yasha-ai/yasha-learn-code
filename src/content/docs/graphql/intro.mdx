...
export default function App() {
  ...
  const fetchData = () => setData(mockGraphQLResponse.data);

  return (
    <div style={{ padding: 20, fontFamily: 'sans-serif' }}>
      <h2>GraphQL: один запрос — точные данные</h2>
      <pre style={{ background: '#1e1e1e', color: '#d4d4d4', padding: 16, borderRadius: 8 }}>{queryText}</pre>
