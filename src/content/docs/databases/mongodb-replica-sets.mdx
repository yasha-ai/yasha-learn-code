...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент Replica Set MongoDB
// Replica Set — группа серверов с копиями данных

class ReplicaNode {
  constructor(name, role) {
    this.name = name;
    this.role = role; // PRIMARY или SECONDARY
    this.data = [];
    this.oplog = [];
  }
  write(doc) {
    if (this.role !== "PRIMARY") throw new Error("Запись только в PRIMARY");
    this.data.push(doc);
    this.oplog.push({ op: "insert", doc, ts: Date.now() });
    return doc;
  }
  replicate(oplog) {
    oplog.forEach(op => {
      if (op.op === "insert") this.data.push(op.doc);
    });
    this.oplog = [...this.oplog, ...oplog];
  }
}

const primary = new ReplicaNode("mongo-1", "PRIMARY");
const secondary1 = new ReplicaNode("mongo-2", "SECONDARY");
const secondary2 = new ReplicaNode("mongo-3", "SECONDARY");

// Запись в Primary
primary.write({ name: "Алиса", age: 25 });
primary.write({ name: "Борис", age: 30 });

// Репликация на Secondary
secondary1.replicate(primary.oplog);
secondary2.replicate(primary.oplog);

console.log("Primary:", primary.data.length, "документов");
console.log("Secondary1:", secondary1.data.length, "документов");
console.log("Secondary2:", secondary2.data.length, "документов");
console.log("Oplog:", primary.oplog.length, "операций");
`
  }}
/>
