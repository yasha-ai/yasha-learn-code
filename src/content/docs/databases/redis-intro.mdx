---
title: "16. Redis: Структуры данных"
sidebar:
  order: 16
---
...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент Redis — in-memory key-value store
...
const redis = new RedisLike();

// SET / GET
redis.set("name", "Алиса");
console.log("GET name:", redis.get("name"));

// SET с TTL
redis.set("temp", "временное", 100);
console.log("GET temp:", redis.get("temp"));

// Хранение объектов (как JSON)
redis.set("user:1", JSON.stringify({ name: "Борис", age: 30 }));
const user = JSON.parse(redis.get("user:1"));
...
redis.set("visits", redis.get("visits") + 1);
console.log("Visits:", redis.get("visits"));
console.log("All keys:", redis.keys());
`
  }}
/>
