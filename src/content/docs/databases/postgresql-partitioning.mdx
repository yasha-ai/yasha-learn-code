---
title: "8. PostgreSQL: Партицирование"
sidebar:
  order: 8
---
...
2. **Размер партиций:**
   - Не слишком маленькие (меньше 1GB)
   - Не слишком большие (больше 100GB)
...
5. **Constraint Exclusion:**
   - Добавляйте CHECK constraints для старых версий PG (до версии 11)

## Когда НЕ использовать Partitioning

❌ **Не подходит для:**
- Маленьких таблиц (меньше 10M строк)
...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент партиционирования PostgreSQL
// Partitioning — разделение большой таблицы на части

// Генерируем данные: заказы за 3 года
const orders = Array.from({ length: 100 }, (_, i) => ({
  id: i + 1,
  amount: Math.round(Math.random() * 10000),
  date: new Date(2022 + Math.floor(i / 34), i % 12, 1)
    .toISOString().slice(0, 10)
}));

// PARTITION BY RANGE (date)
const partitions = {
  orders_2022: orders.filter(o => o.date.startsWith("2022")),
  orders_2023: orders.filter(o => o.date.startsWith("2023")),
  orders_2024: orders.filter(o => o.date.startsWith("2024")),
};

console.log("Всего заказов:", orders.length);
Object.entries(partitions).forEach(([name, data]) => {
  console.log(name + ":", data.length, "записей");
});

// Запрос только в нужную партицию (быстрее!)
console.log("\\nЗаказы за 2023:", partitions.orders_2023.length);
const total2023 = partitions.orders_2023.reduce((s, o) => s + o.amount, 0);
console.log("Сумма за 2023:", total2023);
`
  }}
/>
