---
title: "7. PostgreSQL: Full-Text Search"
sidebar:
  order: 7
---
...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент полнотекстового поиска PostgreSQL

const articles = [
  { id: 1, title: "Введение в JavaScript", body: "JavaScript — язык для веб" },
  { id: 2, title: "React компоненты", body: "React использует компоненты для UI" },
  { id: 3, title: "Node.js сервер", body: "Node.js запускает JS на сервере" },
  { id: 4, title: "Базы данных", body: "PostgreSQL — реляционная база данных" },
];
...
function tsQuery(docs, query) {
  const terms = query.toLowerCase().split(/\\s+/);
  return docs.filter(doc => {
    const tokens = toTsVector(doc.title + " " + doc.body);
    return terms.every(t => tokens.some(tok => tok.includes(t)));
  });
}

console.log("Поиск javascript:", tsQuery(articles, "javascript"));
console.log("Поиск сервер:", tsQuery(articles, "сервер"));
console.log("Поиск база:", tsQuery(articles, "база"));
`
  }}
/>
