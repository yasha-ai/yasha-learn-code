...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент триггеров PostgreSQL
// Триггер — автоматическое действие при изменении данных
...
// Имитация триггера BEFORE UPDATE
function beforeUpdateTrigger(oldRow, newRow) {
  console.log("TRIGGER: Цена изменена с", oldRow.price, "на", newRow.price);
  auditLog.push({
    action: "UPDATE",
    table: "products",
    oldValue: { ...oldRow },
...
function updateProduct(id, changes) {
  ...
  const newRow = { ...products[idx], ...changes };
  products[idx] = beforeUpdateTrigger(oldRow, newRow);
}

updateProduct(1, { price: 95000 });
updateProduct(2, { price: 1800 });

console.log("Продукты:", products);
console.log("Аудит лог:", auditLog);
`
  }}
/>
