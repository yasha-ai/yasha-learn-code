---
title: "5. PostgreSQL: Оконные функции"
sidebar:
  order: 5
---
...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент оконных функций PostgreSQL
// Window functions работают над группой строк

const sales = [
  { dept: "IT", employee: "Алиса", salary: 120000 },
  { dept: "IT", employee: "Борис", salary: 100000 },
  { dept: "IT", employee: "Вика", salary: 110000 },
  { dept: "HR", employee: "Григорий", salary: 90000 },
  { dept: "HR", employee: "Дана", salary: 95000 },
];

// ROW_NUMBER() OVER (PARTITION BY dept ORDER BY salary DESC)
const withRowNum = sales
  .sort((a, b) => b.salary - a.salary)
  .map((row, _, arr) => {
    const deptRows = arr.filter(r => r.dept === row.dept);
    const rank = deptRows.indexOf(row) + 1;
    return { ...row, row_number: rank };
  });
console.log("ROW_NUMBER:", withRowNum);

// SUM(salary) OVER (PARTITION BY dept)
const withDeptTotal = sales.map(row => {
  const deptTotal = sales
    .filter(r => r.dept === row.dept)
    .reduce((s, r) => s + r.salary, 0);
  return { ...row, dept_total: deptTotal };
});
console.log("SUM OVER PARTITION:", withDeptTotal);
`
  }}
/>
