...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент работы с JSON в PostgreSQL
// PostgreSQL поддерживает JSONB — бинарный JSON

const products = [
  { id: 1, name: "Ноутбук", meta: { brand: "Apple", specs: { ram: 16, ssd: 512 } } },
  { id: 2, name: "Телефон", meta: { brand: "Samsung", specs: { ram: 8, ssd: 256 } } },
  { id: 3, name: "Планшет", meta: { brand: "Apple", specs: { ram: 8, ssd: 128 } } },
];

// SELECT * FROM products WHERE meta->'brand' = 'Apple'
const apple = products.filter(p => p.meta.brand === "Apple");
console.log("brand = Apple:", apple.map(p => p.name));

// SELECT meta->'specs'->>'ram' FROM products
const rams = products.map(p => ({
  name: p.name,
  ram: p.meta.specs.ram
}));
console.log("RAM каждого:", rams);

// UPDATE products SET meta = jsonb_set(meta, '{specs,ram}', '32')
const updated = products.map(p =>
  p.name === "Ноутбук"
    ? { ...p, meta: { ...p.meta, specs: { ...p.meta.specs, ram: 32 } } }
    : p
);
console.log("После обновления:", updated[0].meta.specs);
`
  }}
/>
