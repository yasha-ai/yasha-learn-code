...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент индексов PostgreSQL
// Индекс ускоряет поиск, как словарь ускоряет поиск слова

const data = Array.from({ length: 10000 }, (_, i) => ({
  id: i,
  name: "user_" + i,
  city: ["Москва", "Питер", "Казань"][i % 3]
}));

// Без индекса: линейный поиск O(n)
console.time("Без индекса");
const found1 = data.find(u => u.name === "user_9999");
console.timeEnd("Без индекса");

// Создаём "индекс" — Map для быстрого поиска O(1)
const nameIndex = new Map();
data.forEach(u => nameIndex.set(u.name, u));

console.time("С индексом (Map)");
const found2 = nameIndex.get("user_9999");
console.timeEnd("С индексом (Map)");

// Составной индекс (city + name)
const compositeIndex = new Map();
data.forEach(u => {
  const key = u.city + ":" + u.name;
  compositeIndex.set(key, u);
});

console.log("Найден:", found2);
console.log("Размер индекса:", nameIndex.size);
console.log("Составной индекс:", compositeIndex.size, "записей");
`
  }}
/>
