...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент шардинга MongoDB
// Sharding — распределение данных по серверам

class Shard {
  constructor(name) { this.name = name; this.data = []; }
  insert(doc) { this.data.push(doc); }
  find(query) { return this.data.filter(query); }
}

// Shard Key — поле для распределения
function getShardKey(doc, numShards) {
  // Хэш-шардинг по userId
  return doc.userId % numShards;
}

const shards = [new Shard("shard-0"), new Shard("shard-1"), new Shard("shard-2")];
...
docs.forEach(doc => {
  const shardIdx = getShardKey(doc, shards.length);
  shards[shardIdx].insert(doc);
});

shards.forEach(s => {
  console.log(s.name + ":", s.data.length, "документов",
    s.data.map(d => d.name));
});

// Scatter-gather запрос (ко всем шардам)
const allResults = shards.flatMap(s => s.find(d => d.orders > 50));
console.log("\\nOrders > 50 (scatter-gather):", allResults.length);
`
  }}
/>
