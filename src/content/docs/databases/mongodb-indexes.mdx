...
<Playground client:visible
  template="vanilla"
  files={{
    "/index.js": `// JavaScript-эквивалент индексов MongoDB
// createIndex() ускоряет запросы к коллекции

const collection = Array.from({ length: 5000 }, (_, i) => ({
  _id: i,
  name: "user_" + i,
...
// Без индекса — полный скан коллекции
console.time("Без индекса");
collection.find(d => d.email === "user4999@mail.ru");
console.timeEnd("Без индекса");

// Создаём индекс (Map)
const emailIndex = new Map();
collection.forEach(d => emailIndex.set(d.email, d));

console.time("С индексом");
emailIndex.get("user4999@mail.ru");
console.timeEnd("С индексом");

// Compound Index: { status: 1, name: 1 }
const compoundIndex = new Map();
collection.forEach(d => {
  const key = d.status + "|" + d.name;
  compoundIndex.set(key, d);
});

// db.collection.find({ status: "active" })
const activeUsers = collection.filter(d => d.status === "active");
console.log("Active users:", activeUsers.length);
console.log("Размер email индекса:", emailIndex.size);
`
  }}
/>
