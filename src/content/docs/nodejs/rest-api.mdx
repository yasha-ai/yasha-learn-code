...
let nextId = 4;
const initial = [
  { id: 1, title: "Изучить REST", done: true },
  { id: 2, title: "Создать API", done: false },
  { id: 3, title: "Написать тесты", done: false },
];
...
export default function App() {
  ...
  const api = (method, url, body, result) => {
  ...
  const getAll = () => api("GET", "/api/todos", null, { status: 200, data: todos });

  const create = () => {
    if (!newTitle) return;
    const todo = { id: nextId++, title: newTitle, done: false };
    setTodos(prev => [...prev, todo]);
    setNewTitle("");
    api("POST", "/api/todos", { title: newTitle }, { status: 201, data: todo });
  };

  const toggle = (id) => {
    setTodos(prev => prev.map(t => t.id === id ? { ...t, done: !t.done } : t));
    const todo = todos.find(t => t.id === id);
    api("PATCH", \`/api/todos/\${id}\`, null, { status: 200, data: { ...todo, done: !todo?.done } });
  };

  const remove = (id) => {
    setTodos(prev => prev.filter(t => t.id !== id));
    api("DELETE", \`/api/todos/\${id}\`, null, { status: 200, data: { deleted: true } });
  };

  const methodColor = { GET: "#10b981", POST: "#3b82f6", PATCH: "#f59e0b", DELETE: "#ef4444" };
  ...
  return (
    <div style={{ fontFamily: "monospace", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
        ...
        <button onClick={create} style={{ padding: "5px 10px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12 }}>POST</button>
        <button onClick={getAll} style={{ padding: "5px 10px", background: "#10b981", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12 }}>GET all</button>
      </div>
      <div style={{ marginBottom: 12 }}>
        {todos.map(t => (
          <div key={t.id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 0", fontSize: 12 }}>
            <span onClick={() => toggle(t.id)} style={{ cursor: "pointer", color: t.done ? "#10b981" : "#94a3b8" }}>
              {t.done ? "✓" : "○"} {t.title}
            </span>
            <button onClick={() => remove(t.id)} style={{ marginLeft: "auto", padding: "2px 6px", background: "#ef4444", color: "#fff", border: "none", borderRadius: 3, cursor: "pointer", fontSize: 10 }}>DEL</button>
          </div>
        ))}
      </div>
      <div style={{ background: "#0f172a", padding: 10, borderRadius: 6, minHeight: 60 }}>
        {log.map((l, i) => (
          <div key={i} style={{ fontSize: 11, marginBottom: 4 }}>
            <span style={{ color: methodColor[l.method] }}>{l.method}</span> <span style={{ color: "#94a3b8" }}>{l.url}</span> <span style={{ color: "#10b981" }}>{l.status}</span>
            <pre style={{ margin: "2px 0 0", color: "#e2e8f0", fontSize: 10 }}>{l.body}</pre>
          </div>
        ))}
      </div>
    </div>
