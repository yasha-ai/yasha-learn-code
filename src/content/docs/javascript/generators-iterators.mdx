---
title: "84. Генераторы и итераторы"
sidebar:
  order: 86
---
...
<head>
  ...
  <style>
    ...
    .step { color: #cba6f7; } .done { color: #f38ba8; }
    select { background: #313244; color: #cdd6f4; border: 1px solid #45475a; padding: 6px 10px; border-radius: 4px; font-size: 14px; }
  </style>
</head>
<body>
  <h3>⚙️ Generators & Iterators</h3>
  <div class="box">
    <label style="font-size:13px;color:#a6adc8">Генератор: </label>
    <select id="genType" onchange="reset()">
      <option value="fib">Fibonacci</option>
      <option value="range">range(1, 10)</option>
      <option value="infinite">Бесконечный счётчик</option>
    </select>
    <br><br>
    <button id="nextBtn" onclick="step()">▶ next()</button>
    <button onclick="reset()">↺ Сбросить</button>
    <button onclick="runAll()">⏭ Все значения</button>
  </div>
  <div class="output" id="out">// Нажмите next() для получения следующего значения</div>
  <script>
    function* fibonacci() {
      let [a, b] = [0, 1];
    ...
    window.reset = function() {
      ...
      gen = t === 'fib' ? fibonacci() : t === 'range' ? range(1, 10) : counter();
      steps = 0; log = [];
      document.getElementById('nextBtn').disabled = false;
      document.getElementById('out').textContent = '// Генератор сброшен. Нажмите next()';
    }
    window.step = function() {
      const { value, done } = gen.next();
      steps++;
      if (done) {
        log.push(\`  [шаг \${steps}] done: true (генератор завершён)\`);
        document.getElementById('nextBtn').disabled = true;
      } else {
        log.push(\`  [шаг \${steps}] { value: \${value}, done: false }\`);
      }
      document.getElementById('out').textContent = \`gen.next() вызван \${steps} раз:\n\n\${log.join('\\n')}\`;
    }
    window.runAll = function() {
      const t = document.getElementById('genType').value;
      if (t === 'infinite') {
        const g = counter(); const vals = [];
        for (let i = 0; i < 10; i++) vals.push(g.next().value);
        document.getElementById('out').textContent = \`// Бесконечный генератор — берём первые 10:\n[...take(10)] → [\${vals.join(', ')}]\`;
        return;
      }
      const g = t === 'fib' ? fibonacci() : range(1, 10);
      const limit = t === 'fib' ? 10 : 100;
      const vals = []; let r;
      let i = 0;
      do { r = g.next(); if (!r.done) vals.push(r.value); i++; } while (!r.done && i < limit);
      document.getElementById('out').textContent = \`// Все значения генератора \${t}:\n[\${vals.join(', ')}]\`;
      reset();
    }
    reset();
  </script>
