...
<body>
  ...
  <script>
    ...
    window.runScenario = async function(scenario) {
      log.innerHTML = '';
      var codeSuccess = 'async function loadData() ...';
      var codeNetwork = 'try ... catch(e) ...';
      var codeHttp = 'if (!res.ok) throw new Error(...)';
      var codeCustom = 'class ValidationError extends Error ...';
      var codes = { success: codeSuccess, network: codeNetwork, http: codeHttp, custom: codeCustom };
