...
// Mocking — substituting dependencies in tests
...
// Create mock function (like jest.fn())
...
// Real code: notification sending service
function notifyUser(user, sender) {
  if (!user.email) return false;
  sender.send(user.email, 'Hello, ' + user.name + '!');
  return true;
}
...
const user = { name: 'Alexey', email: 'alex@dev.com' };

test('sends notification', () => {
  const result = notifyUser(user, mockSender);
  expect(result).toBe(true);
  expect(mockSender.send).toHaveBeenCalledWith('alex@dev.com', 'Hello, Alexey!');
});

test('does not send without email', () => {
  expect(notifyUser({ name: 'Test' }, mockSender)).toBe(false);
});

console.log('\nMocks isolate tested code from dependencies!');
`
}} />
