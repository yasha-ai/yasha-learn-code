...
// Matchers — методы сравнения в тестах
...
function expect(v) {
  return {
    ...
    toEqual(e) { if (JSON.stringify(v) !== JSON.stringify(e)) throw new Error('not deep equal'); },
    toBeTruthy() { if (!v) throw new Error('expected truthy'); },
    toBeFalsy() { if (v) throw new Error('expected falsy'); },
    toBeNull() { if (v !== null) throw new Error('expected null'); },
    toBeGreaterThan(e) { if (!(v > e)) throw new Error(v + ' not > ' + e); },
    toContain(e) { if (!v.includes(e)) throw new Error('missing: ' + e); },
    toHaveLength(e) { if (v.length !== e) throw new Error('length ' + v.length + ' !== ' + e); },
    toMatch(e) { if (!e.test(v)) throw new Error('no match'); },
  };
}

console.log('=== Equality ===');
test('toBe — строгое ===', () => expect(2 + 2).toBe(4));
test('toEqual — глубокое сравнение', () => expect({ a: 1 }).toEqual({ a: 1 }));

console.log('\n=== Truthiness ===');
test('toBeTruthy', () => expect('hello').toBeTruthy());
test('toBeFalsy', () => expect(0).toBeFalsy());
test('toBeNull', () => expect(null).toBeNull());

console.log('\n=== Numbers ===');
test('toBeGreaterThan', () => expect(10).toBeGreaterThan(5));

console.log('\n=== Strings & Arrays ===');
test('toContain', () => expect([1, 2, 3]).toContain(2));
test('toHaveLength', () => expect('hello').toHaveLength(5));
test('toMatch regex', () => expect('hello@test.com').toMatch(/@/));
`
}} />
