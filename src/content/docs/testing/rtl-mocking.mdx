...
// Mocks in React Testing Library

console.log('=== Mocking in RTL ===\n');

console.log(\`// 1. Module mock
jest.mock('./api', () => ({
...
beforeEach(() => {
  global.fetch = jest.fn(() =>
    Promise.resolve({
      ok: true,
      json: () => Promise.resolve({ name: 'Alexey' }),
    })
  );
});

// 3. Mock useNavigate (React Router)
const mockNavigate = jest.fn();
jest.mock('react-router-dom', () => ({
  ...jest.requireActual('react-router-dom'),
  useNavigate: () => mockNavigate,
...
console.log('\nTip: mock at module level, not inside component');
`
}} />
