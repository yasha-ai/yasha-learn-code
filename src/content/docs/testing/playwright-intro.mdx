...
console.log('=== Playwright: E2E тесты ===\n');
...
test('пользователь может войти', async ({ page }) => {
  ...
  // Заполняем форму
  await page.getByLabel('Email').fill('alex@dev.com');
  await page.getByLabel('Пароль').fill('secret');
  await page.getByRole('button', { name: 'Войти' }).click();
  
  // Проверяем редирект
  await expect(page).toHaveURL('/dashboard');
  await expect(page.getByText('Привет, Алексей!')).toBeVisible();
});\`);

console.log('\nPlaywright поддерживает: Chromium, Firefox, WebKit');
console.log('npx playwright test — запуск');
console.log('npx playwright show-report — отчёт');
`
}} />
