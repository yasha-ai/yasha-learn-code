...
// Jest — популярный фреймворк тестирования

// Мини-реализация Jest API для демонстрации
const results = [];
...
function it(name, fn) {
  try { fn(); results.push({ pass: true, name }); console.log('  ✅ ' + name); }
  catch (e) { results.push({ pass: false, name }); console.log('  ❌ ' + name + ': ' + e.message); }
}

function expect(v) {
  return {
    toBe(e) { if (v !== e) throw new Error('Expected ' + e + ', got ' + v); },
    toContain(e) { if (!v.includes(e)) throw new Error('Not found: ' + e); },
    toThrow() { try { v(); throw new Error('No throw'); } catch(e) { if (e.message === 'No throw') throw e; } },
  };
}
...
describe('reverseString', () => {
  it('переворачивает строку', () => expect(reverseString('hello')).toBe('olleh'));
  it('пустая строка', () => expect(reverseString('')).toBe(''));
});

describe('capitalize', () => {
  it('первая буква заглавная', () => expect(capitalize('hello')).toBe('Hello'));
  it('уже заглавная', () => expect(capitalize('Hello')).toBe('Hello'));
});

console.log('\nTests: ' + results.filter(r => r.pass).length + ' passed, ' + results.length + ' total');
`
}} />
