---
title: "12. React Testing Library"
sidebar:
  order: 11
---
...
console.log('// Философия: тестируйте поведение, не реализацию\n');
...
// Тест
test('показывает приветствие', () => {
  render(<Greeting name="Алексей" />);
  
  // Ищем текст как пользователь
  const heading = screen.getByText('Привет, Алексей!');
  expect(heading).toBeInTheDocument();
  expect(heading.tagName).toBe('H1');
});

test('кнопка меняет текст', () => {
  render(<Counter />);
...
console.log('\n✅ RTL поощряет accessible queries (getByRole, getByText)');
console.log('❌ Избегайте getByTestId — это деталь реализации');
`
}} />
