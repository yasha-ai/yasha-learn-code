...
console.log('// Philosophy: test behavior, not implementation\n');
...
// Test
test('shows greeting', () => {
  render(<Greeting name="Alexey" />);
  
  // Find text as user would
  const heading = screen.getByText('Hello, Alexey!');
...
console.log('\n✅ RTL encourages accessible queries (getByRole, getByText)');
console.log('❌ Avoid getByTestId — it is an implementation detail');
`
}} />
