...
// === Типы тестов: наглядная пирамида ===

const pyramid = [
  { type: 'E2E тесты', count: 3, icon: '🔝', desc: 'Полный сценарий пользователя' },
  { type: 'Integration', count: 8, icon: '🔗', desc: 'Взаимодействие модулей' },
  { type: 'Unit тесты', count: 25, icon: '🧱', desc: 'Отдельные функции' },
];

console.log('=== Пирамида тестирования ===\n');
pyramid.reverse().forEach(level => {
  const bar = '█'.repeat(level.count);
  console.log(level.icon + ' ' + level.type + ' (' + level.count + ')');
  console.log('  ' + bar);
  console.log('  ' + level.desc + '\n');
});

// Пример каждого типа:
console.log('--- Unit: тест одной функции ---');
console.log('expect(formatPrice(1999)).toBe("19.99 ₽")\n');

console.log('--- Integration: модули вместе ---');
console.log('expect(cart.addItem(product)).toChangeTotal()\n');

console.log('--- E2E: пользовательский сценарий ---');
console.log('page.click("Купить") → page.expectURL("/checkout")');
`
}} />
