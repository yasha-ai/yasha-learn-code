...
![User Events](/lessons/user-events.png)
...
// User Events — симуляция действий пользователя

console.log('=== userEvent vs fireEvent ===\n');

console.log(\`// ❌ fireEvent — низкоуровневый
import { fireEvent } from '@testing-library/react';
fireEvent.click(button);

// ✅ userEvent — реалистичный (hover → focus → click)
import userEvent from '@testing-library/user-event';
const user = userEvent.setup();
...
const events = [
  { action: 'user.click(element)', desc: 'Клик (hover + pointerdown + mousedown + focus + pointerup + mouseup + click)' },
  { action: 'user.dblClick(element)', desc: 'Двойной клик' },
  { action: "user.type(input, 'text')", desc: 'Печатает по одному символу (keydown + keypress + keyup)' },
  { action: 'user.clear(input)', desc: 'Очищает поле (select all + delete)' },
  { action: "user.selectOptions(select, ['a'])", desc: 'Выбирает option' },
  { action: 'user.tab()', desc: 'Tab навигация' },
  { action: "user.keyboard('{Enter}')", desc: 'Нажатие клавиши' },
];
