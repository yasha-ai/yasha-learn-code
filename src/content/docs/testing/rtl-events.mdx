...
// User Events — simulating user actions

console.log('=== userEvent vs fireEvent ===\n');

console.log(\`// ❌ fireEvent — low-level
import { fireEvent } from '@testing-library/react';
fireEvent.click(button);

// ✅ userEvent — realistic (hover → focus → click)
import userEvent from '@testing-library/user-event';
const user = userEvent.setup();
...
const events = [
  { action: 'user.click(element)', desc: 'Click (hover + pointerdown + mousedown + focus + pointerup + mouseup + click)' },
  { action: 'user.dblClick(element)', desc: 'Double click' },
  { action: "user.type(input, 'text')", desc: 'Types one character at a time (keydown + keypress + keyup)' },
  { action: 'user.clear(input)', desc: 'Clears field (select all + delete)' },
  { action: "user.selectOptions(select, ['a'])", desc: 'Selects option' },
  { action: 'user.tab()', desc: 'Tab navigation' },
  { action: "user.keyboard('{Enter}')", desc: 'Key press' },
];
