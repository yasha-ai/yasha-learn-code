...
// Async компоненты — waitFor, findBy

console.log('=== Тестирование async компонентов ===\n');

console.log(\`// Компонент загружает данные
function UserList() {
...
// Тест
test('загружает и показывает пользователей', async () => {
  // Мокаем fetch
  global.fetch = jest.fn(() =>
    Promise.resolve({ json: () => [{ id: 1, name: 'Алексей' }] })
  );
  ...
  // Ждём данные — findBy* ждёт до 1000ms
  const item = await screen.findByText('Алексей');
...
console.log('• waitFor(() => expect(...)) — ждёт условие');
console.log('• waitForElementToBeRemoved — ждёт исчезновения');
`
}} />
