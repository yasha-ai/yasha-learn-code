...
// Async components — waitFor, findBy

console.log('=== Testing async components ===\n');

console.log(\`// Component loads data
function UserList() {
...
test('loads and shows users', async () => {
  // Mock fetch
  global.fetch = jest.fn(() =>
    Promise.resolve({ json: () => [{ id: 1, name: 'Alexey' }] })
  );
  ...
  // Wait for data — findBy* waits up to 1000ms
  const item = await screen.findByText('Alexey');
...
console.log('• waitFor(() => expect(...)) — waits for condition');
console.log('• waitForElementToBeRemoved — waits for disappearance');
`
}} />
