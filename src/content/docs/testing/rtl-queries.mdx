...
// RTL Queries — приоритет выбора селекторов

console.log('=== Приоритет RTL Queries ===\n');

const queries = [
  { method: 'getByRole', priority: '1 (лучший)', example: "getByRole('button', { name: 'Отправить' })", why: 'Доступность' },
  { method: 'getByLabelText', priority: '2', example: "getByLabelText('Email')", why: 'Формы' },
  { method: 'getByPlaceholder', priority: '3', example: "getByPlaceholderText('Введите...')", why: 'Если нет label' },
  { method: 'getByText', priority: '4', example: "getByText('Привет!')", why: 'Текстовый контент' },
  { method: 'getByDisplayValue', priority: '5', example: "getByDisplayValue('admin')", why: 'Текущее значение input' },
  { method: 'getByTestId', priority: '6 (крайний)', example: "getByTestId('submit-btn')", why: 'Последний вариант' },
];

queries.forEach(q => {
  console.log(q.priority + '. ' + q.method);
  console.log('   ' + q.example);
  console.log('   Когда: ' + q.why + '\n');
});

console.log('--- Варианты ---');
console.log('getBy    — элемент есть, иначе throw');
console.log('queryBy  — null если нет (для проверки отсутствия)');
console.log('findBy   — async, ждёт появления');
console.log('getAllBy  — несколько элементов');
`
}} />
