...
<Playground client:visible template="vanilla" files={{
  '/index.js': `// Example test code (run with Jest/Vitest)
// RTL Queries — selector priority

console.log('=== RTL Queries Priority ===\n');

const queries = [
  { method: 'getByRole', priority: '1 (best)', example: "getByRole('button', { name: 'Submit' })", why: 'Accessibility' },
  { method: 'getByLabelText', priority: '2', example: "getByLabelText('Email')", why: 'Forms' },
  { method: 'getByPlaceholder', priority: '3', example: "getByPlaceholderText('Enter...')", why: 'If no label' },
  { method: 'getByText', priority: '4', example: "getByText('Hello!')", why: 'Text content' },
  { method: 'getByDisplayValue', priority: '5', example: "getByDisplayValue('admin')", why: 'Current input value' },
  { method: 'getByTestId', priority: '6 (last resort)', example: "getByTestId('submit-btn')", why: 'Last resort' },
];

queries.forEach(q => {
  console.log(q.priority + '. ' + q.method);
  console.log('   ' + q.example);
  console.log('   When: ' + q.why + '\n');
});

console.log('--- Variants ---');
console.log('getBy    — element exists, else throw');
console.log('queryBy  — null if absent (for checking absence)');
console.log('findBy   — async, waits for appearance');
console.log('getAllBy  — multiple elements');
`
}} />
