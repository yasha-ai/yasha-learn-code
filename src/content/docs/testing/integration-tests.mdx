...
// Integration test: Cart + PromoService
console.log('=== Integration: Cart + PromoService ===\n');
const cart = new Cart();
const promo = new PromoService();

cart.add({ name: 'Book', price: 1500 }, 2);
cart.add({ name: 'Course', price: 3000 }, 1);

test('cart calculates total', () => expect(cart.total).toBe(6000));
test('SALE10 gives 10% discount', () => expect(promo.apply(cart, 'SALE10')).toBe(5400));
test('SALE50 works when total >= 5000', () => expect(promo.apply(cart, 'SALE50')).toBe(3000));
test('unknown code — no discount', () => expect(promo.apply(cart, 'INVALID')).toBe(6000));
`
}} />
