...
function fetchUser(id) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (id === 1) resolve({ id: 1, name: 'Alexey' });
...
(async () => {
  ...
  await test('fetchUser returns user', async () => {
    const user = await fetchUser(1);
    expect(user.name).toBe('Alexey');
  });

  await test('fetchUser throws error', async () => {
    await expect(fetchUser(999)).rejects.toThrow();
  });

  await test('delay waits the right time', async () => {
  ...
  console.log('\nAlways use await for async tests!');
})();
`
}} />
