...
test('карточка товара', async ({ page }) => {
  ...
  await expect(card).toHaveScreenshot('product-card.png', {
    maxDiffPixelRatio: 0.01,  // Допуск 1%
  });
});
...
console.log('\n📸 Первый запуск создаёт baseline скриншоты');
console.log('📸 Следующие запуски сравнивают с baseline');
console.log('📸 npx playwright test --update-snapshots — обновить');
`
}} />
