...
// Spies observe calls, Stubs replace implementation
...
// Spy — wrapper around real function
...
console.log('\n=== Stub ===');
const fetchData = createStub({ users: ['Alexey'] });

test('stub returns replaced data', () => {
  const result = fetchData('/api/users');
  expect(result.users[0]).toBe('Alexey');
