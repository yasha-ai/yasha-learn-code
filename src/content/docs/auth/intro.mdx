---
title: "1. Введение в Auth & Security"
sidebar:
  order: 0
---
...
const layers = [
  { name: 'HTTPS', icon: '🔒', desc: 'Шифрование трафика', color: '#22c55e' },
  { name: 'Валидация', icon: '✅', desc: 'Проверка входных данных', color: '#3b82f6' },
  { name: 'Аутентификация', icon: '🔑', desc: 'Проверка личности', color: '#a855f7' },
  { name: 'Авторизация', icon: '🛡️', desc: 'RBAC / ABAC', color: '#f59e0b' },
  { name: 'Хэширование', icon: '🧬', desc: 'bcrypt / Argon2', color: '#ef4444' },
  { name: 'Мониторинг', icon: '📊', desc: 'Аудит и логи', color: '#06b6d4' },
];
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: 'system-ui', padding: 20, background: '#1a1a2e', minHeight: '100vh', color: '#fff' }}>
      <h2 style={{ textAlign: 'center', marginBottom: 8 }}>Defense in Depth</h2>
      <p style={{ textAlign: 'center', color: '#94a3b8', marginBottom: 20 }}>Нажми на слой, чтобы узнать больше</p>
        ...
        {layers.map((l, i) => (
          <div key={i} onClick={() => setActive(active === i ? -1 : i)}
            style={{
              ...
              background: active === i ? l.color + '33' : '#ffffff0d',
              border: '1px solid ' + (active === i ? l.color : '#ffffff1a'),
              transition: 'all 0.2s',
            }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{l.icon} Layer {i + 1}: {l.name}</span>
              <span style={{ color: l.color, fontSize: 12 }}>{active === i ? '▲' : '▼'}</span>
            </div>
            {active === i && (
              <p style={{ margin: '8px 0 0', color: '#cbd5e1', fontSize: 14 }}>{l.desc}</p>
            )}
          </div>
        ))}
      </div>
      {active >= 0 && (
        <p style={{ textAlign: 'center', marginTop: 16, color: '#94a3b8', fontSize: 13 }}>
          Даже если слой "{layers[active].name}" взломан — остальные {layers.length - 1} слоёв защищают систему
        </p>
      )}
    </div>
  );
