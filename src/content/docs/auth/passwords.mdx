...
## Попробуй: Визуализация хэширования
...
export default function App() {
  ...
  const [salt] = useState(crypto.getRandomValues(new Uint8Array(8)).reduce((s,b) => s + b.toString(16).padStart(2,'0'), ''));
  const [hashWithSalt, setHashWithSalt] = useState('');

  const handleChange = async (val) => {
    setPassword(val);
    if (val) {
      setHash(await sha256(val));
      setHashWithSalt(await sha256(val + salt));
    } else {
      setHash('');
      setHashWithSalt('');
    }
  ...
  return (
    <div style={{ fontFamily: 'system-ui', padding: 20, background: '#1a1a2e', minHeight: '100vh', color: '#fff' }}>
      <h2>Хэширование паролей</h2>
      <input
        value={password} onChange={e => handleChange(e.target.value)}
        placeholder="Введи пароль..."
        style={{ width: '100%', padding: 10, borderRadius: 6, border: '1px solid #444', background: '#0f0f23', color: '#fff', fontSize: 16, marginBottom: 16, boxSizing: 'border-box' }}
      />
      {hash && (
        <>
          <div style={{ marginBottom: 12 }}>
            <div style={{ color: '#f59e0b', fontWeight: 600, marginBottom: 4 }}>SHA-256 (без соли):</div>
            <code style={{ fontSize: 11, wordBreak: 'break-all', color: '#94a3b8' }}>{hash}</code>
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ color: '#22c55e', fontWeight: 600, marginBottom: 4 }}>Соль: <code style={{ color: '#06b6d4' }}>{salt}</code></div>
            <div style={{ color: '#22c55e', fontWeight: 600, marginBottom: 4 }}>SHA-256 (с солью):</div>
            <code style={{ fontSize: 11, wordBreak: 'break-all', color: '#94a3b8' }}>{hashWithSalt}</code>
          </div>
          <div style={{ background: '#1e293b', padding: 12, borderRadius: 6, fontSize: 13, color: '#cbd5e1' }}>
            <p style={{ margin: 0 }}>Одинаковые пароли без соли = одинаковые хэши.</p>
            <p style={{ margin: '4px 0 0' }}>С солью — каждый хэш уникален, даже для одного пароля.</p>
            <p style={{ margin: '4px 0 0', color: '#ef4444' }}>В реальности используй bcrypt/Argon2 — SHA-256 слишком быстрый!</p>
          </div>
        </>
      )}
    </div>
