---
title: "15. Best Practices: чеклист"
sidebar:
  order: 14
---
...
## Попробуй: Security Audit Scorecard
...
const checks = [
  { area: 'Пароли', items: [
    { name: 'bcrypt/Argon2 хэширование', points: 15 },
    { name: 'Rate limiting на логин', points: 10 },
  ]},
  { area: 'Сессии', items: [
    { name: 'HttpOnly + Secure + SameSite куки', points: 10 },
    { name: 'JWT с exp, короткий срок', points: 10 },
  ]},
  { area: 'HTTPS', items: [
    { name: 'HTTPS + HSTS', points: 10 },
    { name: 'Security Headers (CSP, X-Frame)', points: 15 },
  ]},
  { area: 'Защита', items: [
    { name: 'XSS: экранирование + CSP', points: 10 },
    { name: 'CSRF: SameSite + токены', points: 10 },
  ]},
  { area: 'Доступ', items: [
    { name: 'RBAC/ABAC авторизация', points: 10 },
  ]},
];
...
export default function App() {
  ...
  const total = checks.flatMap(c => c.items).reduce((s, i) => s + i.points, 0);
  const score = checks.flatMap(c => c.items).reduce((s, i) => s + (checked[i.name] ? i.points : 0), 0);
  const grade = score >= 80 ? 'A+' : score >= 60 ? 'B' : score < 40 ? 'F' : 'C';
  const gradeColor = score >= 80 ? '#22c55e' : score >= 60 ? '#f59e0b' : '#ef4444';
  const verdict = score >= 80 ? 'Отлично!' : score >= 60 ? 'Хорошо, но есть пробелы' : 'Критические проблемы!';

  return (
    <div style={{ fontFamily: 'system-ui', padding: 20, background: '#1a1a2e', minHeight: '100vh', color: '#fff' }}>
      <h2>Security Audit Scorecard</h2>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, background: '#0f0f23', padding: 16, borderRadius: 10 }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: gradeColor + '22', border: '3px solid ' + gradeColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 700, color: gradeColor }}>
          {grade}
        </div>
        <div>
          <div style={{ fontSize: 24, fontWeight: 700 }}>{score} <span style={{ fontSize: 14, color: '#64748b' }}>/ {total}</span></div>
          <div style={{ color: gradeColor, fontSize: 14 }}>{verdict}</div>
        </div>
        <div style={{ marginLeft: 'auto', width: 100, height: 8, background: '#1e293b', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ width: (score/total*100) + '%', height: '100%', background: gradeColor, transition: 'all 0.3s' }} />
        </div>
      </div>
      {checks.map(c => (
        <div key={c.area} style={{ marginBottom: 12 }}>
          <div style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', marginBottom: 4 }}>{c.area}</div>
          {c.items.map(item => (
            <div key={item.name} onClick={() => toggle(item.name)} style={{
              padding: '8px 12px', marginBottom: 4, borderRadius: 6, cursor: 'pointer',
              background: checked[item.name] ? '#22c55e11' : '#0f0f23',
              border: '1px solid ' + (checked[item.name] ? '#22c55e33' : '#ffffff0d'),
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            }}>
              <span style={{ fontSize: 13 }}>
                <span style={{ marginRight: 8 }}>{checked[item.name] ? '☑' : '☐'}</span>
                {item.name}
              </span>
              <span style={{ fontSize: 12, color: '#64748b' }}>+{item.points}</span>
            </div>
          ))}
        </div>
      ))}
