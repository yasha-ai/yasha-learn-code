---
title: "29. color-mix() и относительные цвета"
sidebar:
  order: 30
---
...
<body>
  <h1>color-mix()</h1>
  <div class="box" style="background-color: color-mix(in srgb, red 50%, blue 50%);"></div>

  <h1>Относительные цвета</h1>
  <div class="box" id="relativeColorBox" style="background-color: red;"></div>
  <button id="changeColorButton">Изменить цвет</button>

  <script>
    const box = document.getElementById('relativeColorBox');
    const button = document.getElementById('changeColorButton');
    const colors = [
      'color-mix(in srgb, red 70%, blue 30%)',
      'color-mix(in srgb, blue 60%, green 40%)',
      'color-mix(in hsl, green 50%, red 50%)',
      'color-mix(in srgb, purple 40%, orange 60%)',
    ];
    let idx = 0;
