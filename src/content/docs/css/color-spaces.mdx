...
<head>
  ...
  <style>
    ...
    div { width: 100px; height: 100px; margin: 10px; border: 1px solid white; }
  </style>
</head>
<body>
  <h1>CSS Color Models</h1>
  <div id="rgb"></div>
  <div id="hsl"></div>
  <div id="hwb"></div>
  ...
  <script>
    ...
    const hwbDiv = document.getElementById('hwb');

    rgbDiv.style.backgroundColor = 'rgb(255, 0, 0)';
    hslDiv.style.backgroundColor = 'hsl(120, 100%, 50%)';
    hwbDiv.style.backgroundColor = 'hwb(240 0% 0%)';

    rgbDiv.addEventListener('click', () => { rgbDiv.style.backgroundColor = 'rgb(0, 0, 255)' });
    hslDiv.addEventListener('click', () => { hslDiv.style.backgroundColor = 'hsl(0, 100%, 50%)' });
    hwbDiv.addEventListener('click', () => { hwbDiv.style.backgroundColor = 'hwb(0 0% 0%)' });
  <\/script>
</body>
</html>
`
