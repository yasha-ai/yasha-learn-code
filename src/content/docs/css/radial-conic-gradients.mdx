---
title: "31. Радиальные и конические градиенты"
sidebar:
  order: 32
---
...
<head>
  ...
  <style>
    ...
    .gradient-box { width: 200px; height: 200px; margin: 10px; border: 1px solid #ccc; }
    #radial { background-image: radial-gradient(circle, red, yellow, green); }
    #conic { background-image: conic-gradient(red, yellow, green); }
  </style>
</head>
<body>
  <h1>CSS Gradients</h1>
  <div class="gradient-box" id="radial"></div>
  <p>Radial Gradient</p>
  <div class="gradient-box" id="conic"></div>
  <p>Conic Gradient</p>

  <script>
    const radialBox = document.getElementById('radial');
    radialBox.addEventListener('click', () => {
      radialBox.style.backgroundImage = 'radial-gradient(circle at top left, blue, orange, purple)';
    });

    const conicBox = document.getElementById('conic');
    conicBox.addEventListener('click', () => {
      conicBox.style.backgroundImage = 'conic-gradient(from 45deg, blue, orange, purple)';
    });
  <\/script>
</body>
</html>
