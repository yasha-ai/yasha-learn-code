...
<html>
<head>
  <title>TypeScript Enums & Literals</title>
  <style>
    body { background: #282c34; color: white; font-family: sans-serif; display: flex; flex-direction: column; align-items: center; }
    button { padding: 10px 20px; margin: 5px; border: none; background: #61dafb; color: #282c34; cursor: pointer; }
...
<body>
  ...
  <script>
    ...
    function processOrderStatus(status: OrderStatus) {
      let message = "";
      switch (status) {
        case OrderStatus.PENDING: message = "Заказ ожидает обработки..."; break;
        case OrderStatus.ACTIVE: message = "Заказ активен."; break;
        case OrderStatus.COMPLETE: message = "Заказ завершен."; break;
        default: message = "Неизвестный статус.";
      }
    ...
    Object.values(OrderStatus).forEach(status => {
      if (typeof status === 'string') { // Ensure we only iterate over the enum values
        const button = document.createElement("button");
        button.textContent = status;
        button.onclick = () => processOrderStatus(status as OrderStatus);
