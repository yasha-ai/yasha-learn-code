---
title: "20. Template-driven Forms"
sidebar:
  order: 21
---
...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции Template-Driven Forms (эквивалент Angular)
// Angular: ngModel привязывает поля в шаблоне — минимум логики в TS
// React: useState для каждого поля — аналогичный простой подход
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16, maxWidth: 320 }}>
      ...
      <p style={{ fontSize: 12, color: "#666" }}>
        Как Angular ngModel — логика прямо в шаблоне
      </p>
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: 8 }}>
          <label style={{ fontSize: 13 }}>Имя (ngModel)</label>
          <input value={name} onChange={e => setName(e.target.value)}
            required style={{ width: "100%", padding: "6px 8px", borderRadius: 4, border: "1px solid #ccc" }} />
        </div>
        <div style={{ marginBottom: 8 }}>
          <label style={{ fontSize: 13 }}>Email (ngModel)</label>
          <input value={email} onChange={e => setEmail(e.target.value)}
            type="email" required style={{ width: "100%", padding: "6px 8px", borderRadius: 4, border: "1px solid #ccc" }} />
        </div>
        <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
          <input type="checkbox" checked={agree} onChange={e => setAgree(e.target.checked)} />
          Согласен с условиями
        </label>
        <button type="submit" disabled={!name || !email || !agree}
          style={{ padding: "8px 16px", background: name && email && agree ? "#dd0031" : "#ccc",
            color: "white", border: "none", borderRadius: 4 }}>
          Отправить
        </button>
      </form>
      {submitted && <div style={{ color: "#16a34a", marginTop: 8 }}>Отправлено: {name} ({email})</div>}
    </div>
  );
}
` }} />
