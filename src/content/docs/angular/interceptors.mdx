...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции HTTP Interceptors (эквивалент Angular)
// Angular: перехватчики добавляют заголовки, логируют, обрабатывают ошибки
// Здесь — цепочка мидлваров для fetch

import { useState } from "react";

// Interceptors — функции-мидлвары
const authInterceptor = (req) => ({
  ...req, headers: { ...req.headers, Authorization: "Bearer token123" }
});

const loggingInterceptor = (req, log) => {
  log(">> " + req.method + " " + req.url);
  return req;
};

const errorInterceptor = (response, log) => {
  if (!response.ok) log("ERROR: " + response.status);
  return response;
};

function applyInterceptors(req, interceptors, log) {
  return interceptors.reduce((r, fn) => fn(r, log), req);
}

export default function App() {
  const [logs, setLogs] = useState([]);
  const [active, setActive] = useState({ auth: true, logging: true, error: true });

  const fakeFetch = () => {
    const newLogs = [];
    const log = (msg) => newLogs.push(msg);
    let req = { url: "/api/users", method: "GET", headers: {} };

    const chain = [];
    if (active.auth) chain.push(authInterceptor);
    if (active.logging) chain.push(loggingInterceptor);
    req = applyInterceptors(req, chain, log);

    log("Headers: " + JSON.stringify(req.headers));
    const fakeResponse = { ok: Math.random() > 0.3, status: Math.random() > 0.3 ? 200 : 500 };
    if (active.error) errorInterceptor(fakeResponse, log);
    log("Response: " + fakeResponse.status);
    setLogs(newLogs);
  };
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        {["auth", "logging", "error"].map(k => (
          <label key={k} style={{ marginRight: 12, fontSize: 13 }}>
            <input type="checkbox" checked={active[k]}
              onChange={() => setActive(a => ({ ...a, [k]: !a[k] }))} />
            {" "}{k}Interceptor
          </label>
        ))}
      </div>
      <button onClick={fakeFetch} style={{ padding: "6px 14px", background: "#dd0031", color: "white", border: "none", borderRadius: 4 }}>
        Send Request
      </button>
      <div style={{ marginTop: 10, background: "#f5f5f5", borderRadius: 6, padding: 10, fontSize: 12, fontFamily: "monospace" }}>
        {logs.map((l, i) => <div key={i}>{l}</div>)}
        {!logs.length && <span style={{ color: "#999" }}>Нажми кнопку для запроса</span>}
      </div>
    </div>
  );
}
