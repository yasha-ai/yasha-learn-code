...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции Content Projection (эквивалент Angular)
// Angular: <ng-content> и <ng-content select="...">
// React: props.children — тот же паттерн

import { useState } from "react";

// Аналог Angular компонента с ng-content
function Card({ children, header, footer }) {
  return (
    <div style={{ border: "1px solid #ddd", borderRadius: 8, overflow: "hidden", marginBottom: 12 }}>
      {header && (
        <div style={{ background: "#dd0031", color: "white", padding: "8px 12px", fontWeight: 700 }}>
          {header}
        </div>
      )}
      <div style={{ padding: 12 }}>{children}</div>
      {footer && (
        <div style={{ background: "#f5f5f5", padding: "8px 12px", fontSize: 12, color: "#666" }}>
          {footer}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16 }}>
      ...
      <button onClick={() => setShow(s => !s)} style={{ marginBottom: 12, padding: "6px 12px" }}>
        Toggle footer
      </button>
      <Card
        header="ng-content select='[header]'"
        footer={show ? "ng-content select='[footer]'" : null}
      >
        <p>Этот контент проецируется через <code>&lt;ng-content&gt;</code></p>
        <p style={{ color: "#dd0031" }}>Родитель решает, что показать внутри!</p>
      </Card>
      <p style={{ fontSize: 12, color: "#666" }}>
        Angular ng-content = React children + named slots через props.
      </p>
    </div>
  );
}
