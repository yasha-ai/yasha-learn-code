...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции Change Detection (эквивалент Angular)
// Angular проверяет ВСЕ компоненты при Default стратегии
// OnPush — только при изменении @Input или событиях

import { useState, memo } from "react";

// memo = аналог OnPush стратегии в Angular
const OnPushChild = memo(function OnPushChild({ value }) {
  const renders = ++OnPushChild.count;
  return (
    <div style={{ border: "1px solid #16a34a", borderRadius: 6, padding: 8, marginBottom: 6 }}>
      <strong style={{ color: "#16a34a" }}>OnPush</strong> (memo) — рендеров: {renders}
...
let defaultCount = 0;
function DefaultChild({ value }) {
  const renders = ++defaultCount;
  return (
    <div style={{ border: "1px solid #dd0031", borderRadius: 6, padding: 8, marginBottom: 6 }}>
      <strong style={{ color: "#dd0031" }}>Default</strong> — рендеров: {renders}
      <div style={{ fontSize: 12 }}>value: {value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16 }}>
      ...
      <button onClick={() => setCount(c => c + 1)}
        ...
        Trigger CD (count: {count})
      </button>
      <input value={input} onChange={e => setInput(e.target.value)}
        style={{ padding: "4px 8px" }} placeholder="Измени input" />
      <div style={{ marginTop: 12 }}>
        <DefaultChild value={count} />
        <OnPushChild value={input} />
      </div>
      <p style={{ fontSize: 12, color: "#666" }}>
        Default перерисовывается всегда. OnPush (memo) — только при изменении props.
      </p>
    </div>
  );
}
