...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции Route Resolvers (эквивалент Angular)
// Angular: Resolver загружает данные ДО перехода на страницу
// Здесь — симуляция предзагрузки данных перед показом компонента

import { useState, useEffect } from "react";

// Фейковые "резолверы" — функции предзагрузки данных
const resolvers = {
  user: () => new Promise(r => setTimeout(() => r({ name: "John", role: "admin" }), 800)),
  posts: () => new Promise(r => setTimeout(() => r([
    { id: 1, title: "Angular Signals" },
    { id: 2, title: "Route Resolvers" },
  ]), 1200)),
};
...
export default function App() {
  ...
  const navigate = async (to) => {
    if (to === "home") { setRoute(to); setData(null); return; }
    setLoading(true);
    // Resolver выполняется ДО перехода
    const resolved = await resolvers[to]();
    setData(resolved);
    setRoute(to);
    setLoading(false);
  };
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16 }}>
      <h3>Route Resolvers</h3>
      <nav style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {["home", "user", "posts"].map(r => (
          <button key={r} onClick={() => navigate(r)} disabled={loading}
      ...
      {loading && <div style={{ color: "#dd0031" }}>Resolver загружает данные...</div>}
      {!loading && route === "home" && <p>Главная страница (без резолвера)</p>}
      {!loading && route === "user" && data && (
        <div style={{ background: "#f5f5f5", padding: 12, borderRadius: 6 }}>
          <div>Name: {data.name}</div>
          <div>Role: {data.role}</div>
          <div style={{ fontSize: 11, color: "#888" }}>Данные загружены до рендера!</div>
        </div>
      )}
      {!loading && route === "posts" && data && (
