...
<Playground client:visible template="react" files={{ "/App.js": `// Демонстрация концепции Reactive Forms (эквивалент Angular)
// Angular: FormGroup + FormControl в TypeScript, шаблон только связывается
// React: useReducer — похожий паттерн "логика отдельно от рендера"

import { useReducer } from "react";

const initialState = {
  values: { username: "", email: "", role: "user" },
  errors: {},
  dirty: false,
};
...
function formReducer(state, action) {
  if (action.type === "SET") {
    ...
    if (!values.email.includes("@")) errors.email = "Невалидный email";
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16, maxWidth: 320 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <input placeholder="username (FormControl)"
          value={values.username}
          onChange={e => dispatch({ type: "SET", field: "username", value: e.target.value })}
          style={{ width: "100%", padding: "6px 8px", border: "1px solid " + (errors.username ? "#dd0031" : "#ccc"), borderRadius: 4 }} />
        {errors.username && <span style={{ color: "#dd0031", fontSize: 11 }}>{errors.username}</span>}
      </div>
      <div style={{ marginBottom: 8 }}>
        <input placeholder="email (FormControl)"
          value={values.email}
          onChange={e => dispatch({ type: "SET", field: "email", value: e.target.value })}
          style={{ width: "100%", padding: "6px 8px", border: "1px solid " + (errors.email ? "#dd0031" : "#ccc"), borderRadius: 4 }} />
        {errors.email && <span style={{ color: "#dd0031", fontSize: 11 }}>{errors.email}</span>}
      </div>
      <select value={values.role}
        onChange={e => dispatch({ type: "SET", field: "role", value: e.target.value })}
        style={{ width: "100%", padding: "6px 8px", marginBottom: 8, borderRadius: 4 }}>
        <option value="user">User</option>
        <option value="admin">Admin</option>
      </select>
      <div style={{ display: "flex", gap: 8 }}>
        <button disabled={!valid} style={{ padding: "6px 14px", background: valid ? "#dd0031" : "#ccc", color: "white", border: "none", borderRadius: 4 }}>
          Submit
        </button>
        <button onClick={() => dispatch({ type: "RESET" })} style={{ padding: "6px 14px", borderRadius: 4 }}>
          form.reset()
        </button>
      </div>
      <pre style={{ fontSize: 11, color: "#666", marginTop: 8 }}>
        {JSON.stringify({ values, valid, dirty }, null, 2)}
      </pre>
    </div>
  );
}
