...
Ты освоил систему props в Svelte — от базового синтаксиса до продвинутых паттернов!

**Что запомнить:**

✅ `export let propName = defaultValue` — объявление пропса с дефолтом

✅ `export let propName` — обязательный проп (без дефолта, TypeScript предупредит)

✅ `<Component prop="value" />` — передача строки

✅ `<Component {count} />` — shorthand (имя переменной = имени пропса)

✅ `<Component {...obj} />` — spread props, передать объект как пропсы

✅ `$$props` — ВСЕ переданные пропсы как объект

✅ `$$restProps` — все пропсы, КРОМЕ явно объявленных (идеально для UI-компонентов)

✅ `bind:propName` — двусторонняя привязка

✅ TypeScript `export let name: string` — лучший способ валидации

**Следующий шаг:** разберём **Slots** — как передавать JSX-контент в компоненты! 🎰
