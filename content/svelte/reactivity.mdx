...
Ты освоил реактивность Svelte — один из самых мощных и элегантных механизмов среди всех современных фреймворков!

**Что запомнить:**

✅ `let x = 0` — уже реактивная переменная, обновляй через `x = newValue`

✅ `$: derived = someVar * 2` — вычисляемое значение, пересчитывается автоматически

✅ `$: console.log(someVar)` — реактивный эффект, запускается при изменении зависимостей

✅ `$: { block; of; code; }` — реактивный блок с несколькими операциями

✅ Массивы: всегда **переприсваивай** (`arr = [...arr, item]`), не мутируй методами

✅ Объекты: прямая мутация свойств (`obj.prop = val`) работает, вложенные тоже

✅ Svelte 5: `$state()`, `$derived()`, `$effect()` — новый явный API рун

**Следующий шаг:** разберём **Props** — как передавать данные между компонентами! 📦
