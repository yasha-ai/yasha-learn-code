...
```tsx
// ❌ Не используй setTimeout/setInterval без onCleanup
onMount(() => {
  setInterval(tick, 1000); // Утечка памяти! Таймер не остановится
});

// ✅ Всегда регистрируй очистку
onMount(() => {
  const id = setInterval(tick, 1000);
  onCleanup(() => clearInterval(id));
});

// ❌ Не вызывай onCleanup вне reactive scope
setTimeout(() => onCleanup(() => {}), 100); // Ошибка!

// ✅ onCleanup должен вызываться синхронно внутри эффекта/компонента
createEffect(() => {
  onCleanup(() => {}); // ✅ Синхронно внутри эффекта
});
```

## Итог

| Примитив | Аналог React | Когда |
|---|---|---|
| `onMount` | `useEffect(fn, [])` | DOM-операции, единократные подписки |
| `onCleanup` | cleanup в `useEffect` | Отмена запросов, таймеры, отписки |
| `createEffect` | `useEffect(fn, deps)` | Реактивные побочные эффекты |
| `createRoot` | нет аналога | Изолированный reactive scope |
