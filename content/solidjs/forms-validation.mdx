...
<Playground template="react" files={{ 'App.tsx': `import { useState, useCallback, CSSProperties } from 'react';
...
// ✅ ПРАВИЛЬНО: валидация после blur, обновление после изменения
<input
...
- **Управляемые формы** → `onInput` + `value={signal()}`
- **Неуправляемые формы** → `ref` + `new FormData(form)`
- **`createStore`** — идеально для сложного состояния формы (values + errors + touched)
- **`@modular-forms/solid`** — библиотека для сложных форм с валидацией
- **Server Actions** → `action()` + `"use server"` для форм без JS (progressive enhancement)
- Всегда используй **`onInput`**, а не `onChange` для управляемых полей!
