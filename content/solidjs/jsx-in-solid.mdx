...
## ⚠️ Частые ошибки с JSX в Solid

### Забытые скобки у геттера сигнала

```tsx
// ❌ count без скобок — в JSX вставится сама функция
const [count, setCount] = createSignal(0);
return <p>{count}</p>;   // покажет "() => ..." или пустоту

// ✅ count() — вызов геттера
return <p>{count()}</p>; // покажет актуальное значение
```

### Деструктуризация пропсов

```tsx
// ❌ Деструктуризация разрушает реактивность
function Button({ label, onClick, disabled }) {
  // disabled здесь — это snapshot, не реактивный
  return <button disabled={disabled}>{label}</button>;
}

// ✅ Обращайся через props.xxx
function Button(props) {
  return <button disabled={props.disabled}>{props.label}</button>;
}

// ✅ Или используй splitProps
function Button(props) {
...
```tsx
// ❌ className работает, но это не идиоматично
<div className="box">
...
Ты понял, как JSX работает в Solid.js под капотом!

**Главные отличия от React JSX:**

✅ Компилируется в прямые DOM-операции, не в `React.createElement`

✅ Нет Virtual DOM — реальные элементы с реактивными подписками

✅ `class` вместо `className`, `for` вместо `htmlFor`

✅ Реактивные значения в JSX — всегда функции: `{count()}`, не `{count}`

✅ Для списков используй `<For>` (без `key`), для условий — `<Show>`

✅ Для пропсов — `splitProps`, не деструктуризация

**Следующий шаг:** погружаемся в сигналы — главный реактивный примитив Solid.js! ⚡
