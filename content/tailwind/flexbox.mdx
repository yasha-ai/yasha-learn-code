...
1. Создай навигацию с логотипом слева и кнопками входа/регистрации справа
2. Сделай список уведомлений: иконка + текст + время, выровненные `items-center justify-between`
3. Создай hero-секцию: `flex flex-col items-center justify-center min-h-screen text-center`

### Ключевые моменты

- `flex` + `items-center` + `justify-between` — самый частый паттерн для навигации
- `flex-1` — элемент занимает всё доступное пространство
- `flex-none` — элемент не растягивается (иконки, аватары)
- `flex-col` для вертикальных макетов, `flex-wrap` для адаптивных сеток
