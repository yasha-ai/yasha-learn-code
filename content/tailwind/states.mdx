...
1. Создай список с `group` — при наведении на строку показывай кнопку удаления
2. Сделай форму с `peer-invalid` для показа ошибок валидации
3. Реализуй карточку с эффектом подъёма: `hover:-translate-y-1 hover:shadow-lg transition-all`

### Ключевые моменты

- `hover:`, `focus:`, `active:`, `disabled:` — основные состояния
- `group` + `group-hover:` — стили дочерних элементов при hover на родителе
- `peer` + `peer-invalid:`, `peer-checked:` — стили на основе состояния соседа
- `first:`, `last:`, `odd:`, `even:` — позиционные псевдоклассы

### Полезные ссылки

- [Tailwind Hover, Focus и другие состояния](https://tailwindcss.com/docs/hover-focus-and-other-states)
