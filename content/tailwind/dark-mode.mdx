...
1. Добавь dark mode к существующей карточке: `bg-white dark:bg-gray-800 text-gray-900 dark:text-white`
2. Реализуй кнопку переключения темы с сохранением в `localStorage`
3. Установи `next-themes` в Next.js проекте и настрой системную тему

### Ключевые моменты

- `darkMode: 'class'` — ручное управление (рекомендуется), `'media'` — автоматическое
- Префикс `dark:` — стили только в тёмной теме
- Добавь `transition-colors` для плавного переключения
- `next-themes` — стандартная библиотека для Dark Mode в Next.js
