...
<Playground template="react" files={{ "/App.tsx": `import { useState, useMemo, memo, useRef } from 'react';
...
const ITEMS = Array.from({ length: 5000 }, (_, i) => ({ id: i, score: i % 100 }));

function RenderBadge({ n }: { n: number }) {
  const bg = n > 3 ? '#7f1d1d' : '#14532d';
  const color = n > 3 ? '#fca5a5' : '#86efac';
  return (
    <span style={{ background: bg, color, borderRadius: 12, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>
      {n} {n === 1 ? 'render' : 'renders'}
    </span>
  );
}

function UnoptimizedList({ threshold }: { threshold: number }) {
  const renders = useRenderCount();
  const filtered = ITEMS.filter(x => x.score > threshold);
  return (
    <div style={{ background: '#0f172a', borderRadius: 8, padding: 16, border: '1px solid #374151' }}>
      <div style={{ color: '#f87171', fontSize: 13, marginBottom: 8 }}>Without optimization</div>
      <RenderBadge n={renders} />
      <div style={{ color: '#64748b', fontSize: 12, marginTop: 8 }}>
        Filtered {filtered.length} items — recalculated every render
      </div>
    </div>
  );
}

const OptimizedList = memo(function OptimizedList({ threshold }: { threshold: number }) {
  const renders = useRenderCount();
  const filtered = useMemo(() => ITEMS.filter(x => x.score > threshold), [threshold]);
  return (
    <div style={{ background: '#0f172a', borderRadius: 8, padding: 16, border: '1px solid #374151' }}>
      <div style={{ color: '#4ade80', fontSize: 13, marginBottom: 8 }}>React.memo + useMemo</div>
      <RenderBadge n={renders} />
      <div style={{ color: '#64748b', fontSize: 12, marginTop: 8 }}>
        Filtered {filtered.length} items — memoized result
      </div>
    </div>
  );
});
...
export default function App() {
  ...
  return (
    <div style={{ background: '#0f172a', minHeight: '100vh', padding: 24, fontFamily: 'monospace', color: 'white' }}>
      <h2 style={{ color: '#38bdf8', margin: '0 0 4px' }}>Performance Overview</h2>
      <p style={{ color: '#64748b', fontSize: 13, margin: '0 0 20px' }}>
        Re-render parent without changing threshold — only the unoptimized component re-renders
      </p>
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <button
          onClick={() => setTick(t => t + 1)}
          style={{ background: '#1d4ed8', color: 'white', border: 'none', borderRadius: 6, padding: '8px 16px', cursor: 'pointer', fontSize: 13 }}
        >
        ...
        <label style={{ color: '#94a3b8', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <input
            type="range" min={0} max={95} step={5} value={threshold}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <UnoptimizedList threshold={threshold} />
        <OptimizedList threshold={threshold} />
      </div>
      <div style={{ marginTop: 16, background: '#1e293b', borderRadius: 8, padding: 12, fontSize: 12, color: '#64748b', lineHeight: 1.6 }}>
        React.memo prevents re-render when props are unchanged.
