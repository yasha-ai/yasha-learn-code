...
Route Middleware в Nuxt 3 — мощный инструмент для:

- **Защиты маршрутов** — проверка авторизации и ролей
- **Редиректов** — управление потоком навигации через `navigateTo`
- **Отмены переходов** — `abortNavigation` с кастомными ошибками
- **Глобальных действий** — аналитика, локализация через `*.global.ts`
- **Типобезопасности** — полная поддержка TypeScript
