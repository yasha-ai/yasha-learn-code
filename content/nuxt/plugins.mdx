...
Плагины — фундаментальная часть архитектуры Nuxt 3. Они обеспечивают:

- **Инъекцию зависимостей** — через `provide` и `useNuxtApp()`
- **Изоляцию окружений** — `.client.ts` и `.server.ts`
- **Контроль порядка** — числовые префиксы
- **Vue интеграцию** — регистрация плагинов, директив, компонентов
- **Lifecycle хуки** — подписка на события приложения
