...
```bash
# Через Expo (рекомендуется для начала)
npx create-expo-app MyApp
cd MyApp
npx expo start

# Через React Native CLI (bare)
...
- React Native позволяет создавать **нативные** приложения на JavaScript/TypeScript
- Одна кодовая база для iOS и Android
- Новая архитектура (JSI/Fabric) делает RN быстрее и надёжнее
- Огромная экосистема npm + специализированные библиотеки для мобильной разработки
