...
<body>
  <div class="container">
    ...
    <div class="footer-stats">
      ...
      <button id="clearCompleted">🗑️ Очистить завершённые</button>
    </div>
  </div>
...
function renderTasks() {
  ...
  if (filteredTasks.length === 0) {
    const emptyMessage = currentFilter === 'all' 
      ? '📭 Нет задач<br><small>Добавьте первую задачу выше</small>'
      : currentFilter === 'active'
      ? '✅ Все задачи выполнены!<br><small>Отличная работа!</small>'
      : '📝 Нет завершённых задач<br><small>Начните отмечать задачи</small>';
...
function updateStats() {
  ...
  // Показываем кнопку очистки только если есть завершённые
...
// Добавляем пример задач если список пуст
if (tasks.length === 0) {
  tasks = [
    {
      id: Date.now(),
...
❌ **"Задачи не сохраняются после перезагрузки"**  
→ Проверь вызов `saveTasks()` после каждого изменения

❌ **"Фильтры не работают"**  
→ Убедись что `data-filter` атрибуты совпадают с условиями

❌ **"Удаление не работает"**  
→ Проверь что `onclick="deleteTask(${task.id})"` передаёт правильный ID

❌ **"Input не очищается после добавления"**  
→ Добавь `taskInput.value = ''` в функцию `addTask()`
...
- [MDN: Array methods](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Array)
- [MDN: DOM manipulation](https://developer.mozilla.org/ru/docs/Web/API/Document_Object_Model)

---

**Готово! 🎉 Теперь у тебя есть рабочее приложение и понимание как работает JavaScript в реальном проекте!**
