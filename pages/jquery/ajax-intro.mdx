...
<body>
  <div class="container">
    ...
    <div class="info-box">
      ...
      Загрузка данных <strong>без перезагрузки страницы</strong> 🚀
    </div>
    
    <button id="loadUsers">👥 Загрузить пользователей</button>
    <button id="loadPost">📄 Загрузить пост</button>
  ...
  <script>
    ...
    $('#loadPost').click(function() {
      ...
      .fail(function() {
        $('.loading').removeClass('show');
        $btn.prop('disabled', false);
        $('#content').html('<div style="color: #e74c3c;">Ошибка загрузки поста</div>');
      });
    });
    ...
    $('#showJson').click(function() {
      ...
      $.get('https://jsonplaceholder.typicode.com/posts/1', function(data) {
        ...
        $('#content').html(\`
          <div>
            <h3 style="color: #667eea; margin-top: 30px;">📋 JSON Response:</h3>
            ...
            <div style="margin-top: 15px; padding: 15px; background: #fffbea; border-left: 4px solid #f39c12; border-radius: 6px; font-size: 14px;">
              <strong>💡 JSON</strong> (JavaScript Object Notation) - формат обмена данными между клиентом и сервером.
            </div>
          </div>
        \`);
      });
