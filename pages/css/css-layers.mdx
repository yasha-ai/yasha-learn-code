...
<body>
  <div class="container">
    <h1>CSS Layers Demo</h1>
    <button id="myButton" class="primary-button">Кнопка</button>
    <div class="input-group">
      <label for="buttonText">Текст кнопки:</label>
      <input type="text" id="buttonText" value="Кнопка">
    </div>
  </div>
...
/* Добавляем utility класс к кнопке динамически через JS */
#myButton.rounded {
  @apply rounded; /* Используем @apply, чтобы применить стили из слоя utilities */
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  buttonText.addEventListener('input', updateButtonText);

  // Добавляем класс rounded при загрузке страницы
  button.classList.add('rounded');

  // Пример динамического изменения стилей
  button.addEventListener('click', () => {
    // Меняем цвет фона при каждом клике
    const colors = ['darkblue', 'darkgreen', 'darkred'];
    const currentColor = button.style.backgroundColor;
    let newColor = colors[0];
    if (currentColor === 'darkblue') {
      newColor = colors[1];
    } else if (currentColor === 'darkgreen') {
      newColor = colors[2];
    }
