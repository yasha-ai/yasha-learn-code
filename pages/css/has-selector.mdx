...
![Иллюстрация к уроку](/lessons/css-has-selector.png)
...
<body>
  <div class="container">
    <p>Этот контейнер не содержит кнопки.</p>
  </div>

  <div class="container">
    <p>Этот контейнер содержит кнопку.</p>
...
/* Стилизуем контейнер, только если он содержит кнопку */
.container:has(button) {
  border: 2px solid blue;
  background-color: lightblue;
  color: black;
...
/* Стилизуем input, если форма содержит элемент с классом "error" */
form:has(.error) input {
  border: 1px solid red;
}
...
`,
    "/index.js": `// Пример интерактивности: добавляем/удаляем класс error при клике на кнопку
const myButton = document.getElementById('myButton');
const myForm = document.getElementById('myForm');
const nameInput = document.getElementById('nameInput');

myButton.addEventListener('click', () => {
  // Создаем элемент span.error
  const errorSpan = document.createElement('span');
  errorSpan.className = 'error';
  errorSpan.textContent = 'Поле обязательно для заполнения';

  // Проверяем, есть ли уже span.error в форме
  const existingError = myForm.querySelector('.error');

  if (!existingError) {
    // Добавляем span.error в форму
    myForm.appendChild(errorSpan);

    // Добавляем обработчик события input для удаления ошибки при вводе текста
    nameInput.addEventListener('input', function inputHandler() {
      myForm.removeChild(errorSpan);
      nameInput.removeEventListener('input', inputHandler); // Удаляем обработчик после первого срабатывания
    });
  }
});
`
