...
`,
  ...
  updateGridButton.addEventListener('click', () => {
    ...
    // Обновляем стили grid-container
    gridContainer.style.gridTemplateColumns = \`repeat(\${'}columns\${'}, 1fr)\`;
    gridContainer.style.gridTemplateRows = \`repeat(\${'}rows\${'}, 100px)\`;

    // Добавляем или удаляем элементы, чтобы соответствовать новому размеру сетки
    const currentItems = gridContainer.children.length;
    const targetItems = columns * rows;

    if (targetItems > currentItems) {
      // Добавляем новые элементы
      for (let i = currentItems + 1; i <= targetItems; i++) {
      ...
      for (let i = currentItems; i > targetItems; i--) {
