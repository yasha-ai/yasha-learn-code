...
<head>
  ...
  <title>CSS Grid: Именованные линии</title>
...
<body>
  ...
  <div class="controls">
    <label for="sidebarWidth">Ширина сайдбара:</label>
    <input type="range" id="sidebarWidth" min="100" max="300" value="200">
    <span id="sidebarWidthValue">200px</span>
  </div>
...
// Функция для обновления ширины сайдбара
function updateSidebarWidth() {
  const width = sidebarWidthInput.value + 'px';
  sidebarWidthValue.textContent = width;

  // Обновляем grid-template-columns, используя именованные линии
  container.style.gridTemplateColumns = '[sidebar-start] ' + width + ' [sidebar-end main-start] auto [main-end]';
}

// Добавляем слушатель событий для изменения ширины сайдбара
sidebarWidthInput.addEventListener('input', updateSidebarWidth);

// Инициализируем ширину сайдбара при загрузке страницы
updateSidebarWidth();
`
  }}
  options={{
