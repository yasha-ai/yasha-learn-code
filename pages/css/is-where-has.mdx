...
![Иллюстрация к уроку](/lessons/css-is-where-has.png)
...
<head>
  ...
  <title>Псевдоклассы :is(), :where(), :has()</title>
...
<body>
  <main>
    ...
    <button id="toggle-image">Добавить/Удалить изображение</button>
  </main>
...
`,
  ...
  const withImageDiv = document.getElementById('with-image');
  const noImageDiv = document.getElementById('no-image');

  toggleImageButton.addEventListener('click', () => {
    // Проверяем, есть ли изображение в div с id 'with-image'
    const image = withImageDiv.querySelector('img');

    if (image) {
      // Если изображение есть, удаляем его и добавляем текст
      withImageDiv.innerHTML = '<p>Текст с картинкой</p>';
    } else {
      // Если изображения нет, добавляем его
      withImageDiv.innerHTML = '<img src="https://via.placeholder.com/150" alt="Изображение"><p>Текст с картинкой</p>';
    }
  });
});`
  }}
