...
- `anchor-name: --name` — объявляет элемент якорем
- `position-anchor: --name` — привязывает плавающий элемент к якорю
- `anchor(top|bottom|left|right|center)` — координаты якоря в свойствах позиции
- `anchor-size(width|height)` — размеры якоря
- `inset-area` — новый способ позиционирования через сетку 3×3
- `@position-try` + `position-try-fallbacks` — автоматический flip при выходе за viewport
- **Поддержка**: Chrome 125+, Edge 125+. Safari/Firefox — в разработке
