...
flexDirectionSelect.addEventListener('change', updateContainerStyles);
justifyContentSelect.addEventListener('change', updateContainerStyles);
alignItemsSelect.addEventListener('change', updateContainerStyles);
flexWrapSelect.addEventListener('change', updateContainerStyles);

// Инициализируем стили при загрузке страницы
