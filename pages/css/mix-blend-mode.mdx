...
<head>
  ...
  <style>
    body { background: #282c34; color: white; font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; }
    .container { position: relative; width: 200px; height: 200px; }
    .base { width: 100%; height: 100%; background: linear-gradient(to bottom right, red, blue); }
    .blend { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: yellow; mix-blend-mode: multiply; opacity: 0.7; cursor: pointer;}
  </style>
</head>
<body>
  <div class="container">
  ...
  <script>
    const blendDiv = document.getElementById('blend');
    blendDiv.addEventListener('click', () => {
      const modes = ['multiply', 'screen', 'overlay', 'darken', 'lighten'];
      const currentMode = blendDiv.style.mixBlendMode;
      const currentIndex = modes.indexOf(currentMode);
      const nextIndex = (currentIndex + 1) % modes.length;
      blendDiv.style.mixBlendMode = modes[nextIndex];
    });
  <\/script>
</body>
</html>
