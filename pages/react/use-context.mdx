...
<Sandpack
  template="react"
  files={{
    "/App.tsx": `import React, { createContext, useContext, useState } from 'react';
  ...
  return (
    <ThemeProvider>
      <div style={{
        ...
        backgroundColor: useContext(ThemeContext).theme === 'dark' ? '#333' : '#f0f0f0',
        color: useContext(ThemeContext).theme === 'dark' ? '#fff' : '#000',
        minHeight: '200px',
        borderRadius: '5px'
      }}>
