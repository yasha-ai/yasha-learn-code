...
<Sandpack
  template="react"
  files={{
    "/App.tsx": `import React, { useRef, useReducer, useEffect } from 'react';
...
function App() {
  // useRef для доступа к input элементу
  const inputRef = useRef<HTMLInputElement>(null);

  // useReducer для управления состоянием счетчика
  const [state, dispatch] = useReducer(reducer, { count: 0 });

  // useRef для хранения предыдущего значения счетчика (не вызывает перерисовку)
  const prevCountRef = useRef(state.count);

  useEffect(() => {
    // Обновляем предыдущее значение счетчика после каждого рендера
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#282c34', color: 'white', borderRadius: '5px' }}>
      <h1>useRef и useReducer</h1>

      {/* useRef: Фокус на input */}
      <div>
        <label htmlFor="myInput">Input с фокусом:</label>
        <input ref={inputRef} type="text" id="myInput" style={{ margin: '5px' }} />
