...
<Playground
  template="react"
  files={{
    "/App.tsx": `import React, { useReducer, useRef } from 'react';

// Reducer для управления сложным состоянием (списком задач)
...
export default function App() {
  const [tasks, dispatch] = useReducer(taskReducer, ['Изучить useRef', 'Понять useReducer']);
  
  // useRef для прямого доступа к DOM-элементу input
  const inputRef = useRef(null);
