...
![Иллюстрация к уроку](/lessons/html-web-components-best-practices.png)
...
my-counter span {
  font-size: 1.1em;
    ...
    "/index.js": `// MyButton Web Component
class MyButton extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' }); // Создаем Shadow DOM
...
class MyCounter extends HTMLElement {
  ...
  increment() {
    ...
    this.dispatchEvent(new CustomEvent('count-updated', {
      ...
      composed: true // Разрешаем событию пересекать Shadow DOM
    }));
  }
}
...
// Слушаем кастомное событие на document
document.addEventListener('count-updated', (event) => {
  console.log('Счетчик обновлен:', event.detail.count);
});
`
  }}
  options={{
