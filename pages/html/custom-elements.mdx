...
<body>
  ...
  <my-profile-card id="dynamic-card" name="Анна Петрова" title="Дизайнер"></my-profile-card>

  <label for="name-input">Имя:</label>
  <input type="text" id="name-input" value="Анна Петрова">
...
input[type=text] {
  ...
  color: #eee;
    ...
    "/index.js": `// 1. Определяем класс, расширяющий HTMLElement
class MyGreeting extends HTMLElement {
  constructor() {
    super(); // Обязательно вызываем super() в конструкторе
    ...
    // Создаем элемент параграфа и добавляем текст
    const p = document.createElement('p');
    p.textContent = 'Привет, мир! Это кастомный элемент.';
...
// Интерактивность: обновление имени карточки по кнопке
