...
<Sandpack
  ...
  files={{
    '/index.js': `index.js
// Демонстрация Nullish Coalescing Operator (??)
...
function createTable(data) {
  ...
  ['Значение', 'С ??', 'С ||', 'Разница?'].forEach(text => {
  ...
  data.forEach(([value, label]) => {
    ...
    const valueWithNullish = value ?? 'DEFAULT';
...
theme: \${settings.theme || 'light'}
notifications: \${settings.notifications || true}
volume: \${settings.volume || 50} ⚠️ 0 заменено на 50!
autoplay: \${settings.autoplay || true} ⚠️ false заменено на true!
  </pre>
\`;
  ...
  <pre style="background: white; padding: 10px; border-radius: 5px;">
{
  ...
  <p><strong>Возраст:</strong> \${user.age ?? 'Неизвестно'} (\${user.age} корректно!)</p>
  <p><strong>Очки:</strong> \${user.score ?? 'Еще не играл'}</p>
