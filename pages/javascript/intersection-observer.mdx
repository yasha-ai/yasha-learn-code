...
app.innerHTML = \`
  <h2>👁️ Intersection Observer API</h2>
  ...
  <div id="scrollArea" style="height: 400px; overflow-y: scroll; border: 2px solid #ddd; border-radius: 8px; padding: 20px;">
    ...
    <div class="observe-box" data-id="1" style="margin: 20px 0; padding: 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 8px; text-align: center; opacity: 0.3; transform: translateY(20px); transition: all 0.5s;">
      <h3 style="margin: 0;">📦 Блок 1</h3>
      <p style="margin: 10px 0 0 0;">Анимация при появлении</p>
    </div>
    ...
    <div class="observe-box" data-id="2" style="margin: 20px 0; padding: 30px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border-radius: 8px; text-align: center; opacity: 0.3; transform: translateY(20px); transition: all 0.5s;">
      <h3 style="margin: 0;">🎨 Блок 2</h3>
      <p style="margin: 10px 0 0 0;">Lazy load стиль</p>
    </div>
    ...
    <div class="observe-box" data-id="3" style="margin: 20px 0; padding: 30px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; border-radius: 8px; text-align: center; opacity: 0.3; transform: translateY(20px); transition: all 0.5s;">
      <h3 style="margin: 0;">🚀 Блок 3</h3>
      <p style="margin: 10px 0 0 0;">Появляется плавно</p>
    </div>
...
function initObserver() {
  ...
  const options = {
    root: scrollArea, // Наблюдаем внутри scrollArea
    rootMargin: '0px',
    threshold: threshold
  };
  ...
  observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        log(\`✅ Блок \${id} видим (\${Math.round(entry.intersectionRatio * 100)}%)\`, '#4CAF50');
      } else {
        // Элемент не видим
        entry.target.style.opacity = '0.3';
        entry.target.style.transform = 'translateY(20px)';
        log(\`❌ Блок \${id} скрыт\`, '#999');
      }
    });
  }, options);
  
  // Начинаем наблюдение за всеми блоками
  document.querySelectorAll('.observe-box').forEach(box => {
    observer.observe(box);
  });
  
  log(\`🔧 Observer создан (threshold: \${Math.round(threshold * 100)}%)\`, '#2196F3');
}
...
resetBtn.addEventListener('click', () => {
  ...
  output.innerHTML = '';
  log('🔄 Прокрутка сброшена', '#FF9800');
});

// Инициализация
log('👁️ Intersection Observer готов!', '#673AB7');
log('', '#333');
log('💡 Настройки:', '#666');
log('  • root: scrollArea (область наблюдения)', '#666');
log('  • threshold: % видимости для срабатывания', '#666');
log('  • rootMargin: 0px (отступы)', '#666');
log('', '#333');
log('📜 Прокрутите область ниже, чтобы увидеть эффект', '#FF9800');
