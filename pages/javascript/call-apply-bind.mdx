...
<Sandpack
  template="vanilla"
  files={{
    "/index.js": `// Демонстрация call, apply, bind
...
function greet(greeting, punctuation) {
  return \`\${greeting}, меня зовут \${this.name}, мне \${this.age} лет, я из города \${this.city}\${punctuation}\`;
}

// Демо call
document.getElementById('callBtn').addEventListener('click', () => {
  const result = greet.call(person1, 'Привет', '!');
  document.getElementById('callResult').textContent = result;
});

// Демо apply
document.getElementById('applyBtn').addEventListener('click', () => {
  const result = greet.apply(person2, ['Здравствуйте', '.']);
...
document.getElementById('bindBtn').addEventListener('click', () => {
  const result = greetPerson1('Добрый день', '!');
  document.getElementById('bindResult').textContent = result;
});

// Практический пример: заимствование методов
document.getElementById('borrowBtn').addEventListener('click', () => {
  ...
  document.getElementById('borrowResult').textContent = 
    \`Array-like объект превращен в массив: [\${realArray.join(', ')}]\`;
});`,
...
<body>
  <div class="container">
    <h2>🎯 call, apply, bind Demo</h2>
    
    <div class="demo-section">
      <h3>1️⃣ <code>.call()</code> - передача аргументов отдельно</h3>
      <button id="callBtn">Вызвать с помощью call()</button>
      <div class="result" id="callResult">Нажмите кнопку...</div>
    </div>

    <div class="demo-section">
      <h3>2️⃣ <code>.apply()</code> - передача аргументов массивом</h3>
      <button id="applyBtn">Вызвать с помощью apply()</button>
      <div class="result" id="applyResult">Нажмите кнопку...</div>
    </div>

    <div class="demo-section">
      <h3>3️⃣ <code>.bind()</code> - создание новой функции с контекстом</h3>
      <button id="bindBtn">Вызвать связанную функцию</button>
      <div class="result" id="bindResult">Нажмите кнопку...</div>
    </div>

    <div class="demo-section">
      <h3>4️⃣ Заимствование методов</h3>
      <button id="borrowBtn">Преобразовать array-like в массив</button>
      <div class="result" id="borrowResult">Нажмите кнопку...</div>
    </div>
  </div>
