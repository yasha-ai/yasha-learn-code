...
<head>
  ...
  <style>
    ...
    input { background: #313244; color: #cdd6f4; border: 1px solid #45475a; padding: 5px 8px; border-radius: 4px; font-size: 13px; width: 100px; margin-right: 6px; }
    label { font-size: 13px; margin-right: 6px; }
  </style>
</head>
<body>
  <div class="box">
    <h3>Proxy — валидация и логирование</h3>
    <label>Имя:</label><input id="name" value="Алиса">
    <label>Возраст:</label><input id="age" type="number" value="25" style="width:60px">
    <br><br>
    <button onclick="setProps()">Установить через Proxy</button>
    <button onclick="getProps()">Прочитать свойства</button>
    <button onclick="setBadAge()">Ввести -5 (невалидный)</button>
  </div>
  ...
  <script>
      ...
      set: function(target, prop, value) {
        if (prop === 'age') {
          if (typeof value !== 'number' || value < 0 || value > 150) {
            lines.push('[Proxy SET] ОТКАЗАНО: age=' + value + ' (должен быть 0-150)');
            render(); return false;
          }
        }
        lines.push('[Proxy SET] ' + prop + ' = ' + value);
        target[prop] = value; render(); return true;
      },
      get: function(target, prop) {
        lines.push('[Proxy GET] ' + prop + ' -> ' + target[prop]);
        render(); return target[prop];
      }
    };
    ...
    function setProps() {
      lines = [];
      proxy.name = document.getElementById('name').value;
      proxy.age = parseInt(document.getElementById('age').value) || 0;
    }
    function getProps() {
      var n = proxy.name;
      var a = proxy.age;
    ...
    function setBadAge() {
      lines = [];
      try { proxy.age = -5; } catch(e) { lines.push('Исключение: ' + e.message); }
      render();
    }
  </script>
</body>
