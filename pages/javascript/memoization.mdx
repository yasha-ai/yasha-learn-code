...
// Обычная тяжелая функция
function expensiveOperation(n) {
  regularCallCount++;
  console.log(\`[Обычная] Вычисляем для \${n}...\`);
...
function memoize(func) {
  ...
  return function(...args) {
    ...
    if (cache[key] !== undefined) {
      console.log(\`[Мемо] Результат из кэша для \${args}\`);
      return cache[key];
    }
    
    memoCallCount++;
    console.log(\`[Мемо] Вычисляем для \${args}...\`);
    const result = func(...args);
    cache[key] = result;
    return result;
  };
...
title.textContent = 'Демонстрация мемоизации';
document.body.appendChild(title);

// Тест 1: Обычная функция
const h3_1 = document.createElement('h3');
h3_1.textContent = '1. Обычная функция (без мемоизации)';
document.body.appendChild(h3_1);

const startRegular = performance.now();
const r1 = expensiveOperation(5);
const r2 = expensiveOperation(5); // Повторный вызов
const r3 = expensiveOperation(5); // Еще раз
const endRegular = performance.now();

const div1 = document.createElement('div');
div1.innerHTML = \`
  <p>Результат: \${r1}</p>
  <p><strong>Вызовов функции:</strong> \${regularCallCount}</p>
  <p><strong>Время:</strong> \${Math.round(endRegular - startRegular)}ms</p>
  <p style="color: #f44336;">⚠️ Каждый вызов выполняет полный расчет!</p>
\`;
div1.style.backgroundColor = '#ffebee';
div1.style.padding = '15px';
div1.style.borderRadius = '5px';
div1.style.marginBottom = '20px';
document.body.appendChild(div1);

// Тест 2: Мемоизированная функция
const h3_2 = document.createElement('h3');
h3_2.textContent = '2. Мемоизированная функция';
document.body.appendChild(h3_2);

const startMemo = performance.now();
const m1 = memoizedOperation(5);
const m2 = memoizedOperation(5); // Повторный вызов - из кэша!
const m3 = memoizedOperation(5); // Еще раз - из кэша!
const m4 = memoizedOperation(10); // Новое значение
const endMemo = performance.now();

const div2 = document.createElement('div');
div2.innerHTML = \`
  <p>Результат для 5: \${m1}</p>
  <p>Результат для 10: \${m4}</p>
  <p><strong>Реальных вычислений:</strong> \${memoCallCount}</p>
  <p><strong>Время:</strong> \${Math.round(endMemo - startMemo)}ms</p>
  <p style="color: #4CAF50;">✓ Повторные вызовы используют кэш!</p>
\`;
div2.style.backgroundColor = '#e8f5e9';
...
h3_3.textContent = '3. Сравнение эффективности';
document.body.appendChild(h3_3);

const speedup = ((endRegular - startRegular) / (endMemo - startMemo)).toFixed(2);

const div3 = document.createElement('div');
div3.innerHTML = \`
  <p><strong>Обычная:</strong> \${regularCallCount} вычислений, \${Math.round(endRegular - startRegular)}ms</p>
  <p><strong>Мемоизированная:</strong> \${memoCallCount} вычислений, \${Math.round(endMemo - startMemo)}ms</p>
  <p style="font-size: 1.2em; color: #2196F3;">
    <strong>Ускорение: ~\${speedup}x</strong>
  </p>
\`;
div3.style.backgroundColor = '#e3f2fd';
div3.style.padding = '15px';
div3.style.borderRadius = '5px';
document.body.appendChild(div3);

console.log('Проверьте консоль для деталей вычислений');`
  }}
/>
