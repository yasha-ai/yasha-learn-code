...
### Процесс асинхронной итерации

```mermaid
flowchart TD
    Start[for await...of] --> CallNext[Вызов iterator.next()]
    CallNext --> Wait[Ожидание Promise]
    Wait --> Check{done?}
    Check -- No --> Yield[Получение value]
    Yield --> Body[Выполнение тела цикла]
    Body --> CallNext
    Check -- Yes --> End[Завершение цикла]

    style Wait fill:#fff4dd,stroke:#d4a017
    style Body fill:#e1f5fe,stroke:#01579b
```
*Алгоритм работы цикла `for await...of`.*
