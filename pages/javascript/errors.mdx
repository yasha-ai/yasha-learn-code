...
const demos = {
  basicTryCatch: () => {
    clearOutput();
    log('=== 1️⃣ Базовый try...catch ===', 'info');
  ...
  customError: () => {
    ...
    function validateInput(username) {
      if (username.length < 3) {
        throw new ValidationError("Имя слишком короткое (мин. 3 символа)", "username");
      }
      return true;
    }
    ...
    try {
      ...
      log('✅ Валидация пройдена!', 'success');
    } catch (error) {
    ...
    try {
      ...
      if (error instanceof ValidationError) {
        log(\`❌ \${error.name} для поля "\${error.field}": \${error.message}\`, 'error');
      }
    }
  },
    ...
    function processFile(filename) {
      ...
      try {
        ...
        log(\`📂 \${fileHandle} открыт\`, 'info');
        
        if (filename === "broken.txt") {
          throw new Error("Ошибка чтения файла!");
        }
        log(\`✅ Файл успешно обработан\`, 'success');
        return true;
      } catch (error) {
        log(\`❌ Ошибка: \${error.message}\`, 'error');
        return false;
        ...
        if (fileHandle) {
          log(\`🔒 \${fileHandle} закрыт (finally всегда выполняется!)\`, 'warning');
        }
      }
    }
  ...
  promiseError: () => {
    ...
    fetchData("http://good-api.com")
      .then(data => log(\`✅ \${data}\`, 'success'))
      .catch(error => log(\`❌ Promise error: \${error.message}\`, 'error'));
  ...
  asyncError: async () => {
    clearOutput();
    log('=== 5️⃣ Async/await ошибки ===', 'info');
    
    function fetchData(url) {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          if (url === "http://error-api.com") {
            return reject(new Error("Сервер недоступен"));
          }
          resolve(\`Данные с \${url}\`);
        }, 500);
      });
    ...
    async function getData(url) {
      try {
        log(\`⏳ Запрос к \${url}...\`, 'info');
        const data = await fetchData(url);
        log(\`✅ \${data}\`, 'success');
      } catch (error) {
        log(\`❌ Async error: \${error.message}\`, 'error');
      }
    }
...
log('👆 Выберите пример обработки ошибок', 'info');`
  }}
/>
