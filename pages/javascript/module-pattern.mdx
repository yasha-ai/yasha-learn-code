...
<body>
  <h2>Revealing Module Pattern — Счётчик</h2>
...
h2,h3 { color: #89b4fa; }
.counter-display { font-size: 64px; text-align: center; color: #a6e3a1; font-weight: bold; padding: 20px; background: #313244; border-radius: 12px; margin: 16px 0; }
.controls { text-align: center; margin: 12px 0; }
button { background: #89b4fa; color: #1e1e2e; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; margin: 4px; font-size: 18px; font-weight: bold; }
.op { background: #313244; border-radius: 4px; padding: 6px 12px; margin: 3px 0; font-family: monospace; font-size: 13px; }`,
    "/index.js": `// Revealing Module Pattern
const counter = (function() {
  let count = 0
  const history = []
  
  function record(op) {
    history.push({ op, value: count, time: new Date().toLocaleTimeString() })
  }

  function increment(by = 1) { count += by; record(\`+\${by}\`); return count }
  function decrement(by = 1) { count -= by; record(\`-\${by}\`); return count }
  function reset() { count = 0; record('reset'); return count }
  function getCount() { return count }
  function getHistory() { return [...history] }

  return { increment, decrement, reset, getCount, getHistory }
})()
...
function update() {
  ...
  const h = counter.getHistory()
  historyEl.innerHTML = h.slice(-5).reverse().map(e =>
    \`<div class="op">\${e.time} — \${e.op} → \${e.value}</div>\`
  ).join('')
}

document.getElementById('inc').addEventListener('click', () => { counter.increment(); update() })
document.getElementById('dec').addEventListener('click', () => { counter.decrement(); update() })
document.getElementById('reset').addEventListener('click', () => { counter.reset(); update() })
`
  }}
