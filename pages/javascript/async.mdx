...
<body>
  <div class="container">
    ...
    <button id="fetchButton">Получить данные с сервера</button>
...
`,
    "/index.js": `// Пример использования асинхронности с fetch API и таймером
...
async function fetchData() {
  try {
    dataContainer.textContent = 'Загрузка данных...';
    ...
    const data = await response.json();
    dataContainer.textContent = \`Заголовок: \${'}data.title'}\`; // Отображаем заголовок полученных данных
  } catch (error) {
    console.error('Ошибка при получении данных:', error);
    dataContainer.textContent = 'Произошла ошибка при загрузке данных.';
  }
}
