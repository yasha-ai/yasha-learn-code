...
<head>
  ...
  <title>Composer в JavaScript (пример)</title>
  <link rel="stylesheet" href="/styles.css">
</head>
<body>
  <div class="container">
    <h1>Composer в JavaScript</h1>
    <p>Этот пример демонстрирует концепции Composer (управление зависимостями, повторное использование кода) в JavaScript.</p>

    <h2>Обработка данных</h2>
    <label for="dataInput">Введите данные (через запятую):</label>
    <input type="text" id="dataInput" placeholder="apple,banana,orange">
    ...
    <label for="filterInput">Введите минимальную длину слова:</label>
    <input type="number" id="filterInput" value="5">
    <button id="filterButton">Отфильтровать</button>

    <div id="filteredOutput">
      <h3>Отфильтрованный результат:</h3>
...
li {
  margin-bottom: 5px;
    ...
    "/index.js": `// Функции для обработки данных (аналог библиотек в Composer)
...
// Функция для обработки массива данных
function processData(data) {
...
// Функция для фильтрации массива по длине строки
function filterData(data, minLength) {
  return data.filter(item => item.length >= minLength);
}
...
processButton.addEventListener('click', () => {
  ...
  const data = inputValue.split(',').map(item => item.trim()); // Преобразуем строку в массив
...
// Обработчик нажатия на кнопку "Отфильтровать"
filterButton.addEventListener('click', () => {
  const inputValue = dataInput.value;
  const data = inputValue.split(',').map(item => item.trim());
  const minLength = parseInt(filterInput.value);

  const filteredData = filterData(data, minLength);

  // Очищаем список отфильтрованных результатов
  filteredList.innerHTML = '';

  // Выводим отфильтрованные результаты в список
