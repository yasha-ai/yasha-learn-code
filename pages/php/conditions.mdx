...
<body>
  <div class="container">
    ...
    <div id="age-check">
      ...
      <button id="check-age-button">Проверить</button>
      <p id="age-result"></p>
    </div>

    <div id="grade-calculator">
      <h2>Калькулятор оценок</h2>
      <label for="score">Введите ваш балл:</label>
      <input type="number" id="score" min="0" max="100" value="85">
      <button id="calculate-grade-button">Рассчитать оценку</button>
      <p id="grade-result"></p>
    </div>

    <div id="daytime-greeting">
      <h2>Приветствие по времени суток</h2>
      <p id="greeting"></p>
    </div>
  </div>
...
#age-check, #grade-calculator, #daytime-greeting {
  margin-bottom: 20px;
  padding: 10px;
  border: 1px solid #555;
  border-radius: 5px;
...
calculateGradeButton.addEventListener('click', () => {
  ...
  gradeResult.textContent = "Ваша оценка: " + grade;
});
