...
<body>
  <div class="container">
    <h1>CRUD приложение на JavaScript</h1>

    <div id="input-area">
      <input type="text" id="item-name" placeholder="Название элемента">
...
`,
    "/index.js": `// Массив для хранения элементов
let items = [];
...
// Функция для отображения списка элементов в HTML
function renderList() {
  const itemList = document.getElementById('item-list');
  itemList.innerHTML = ''; // Очищаем список перед перерисовкой

  items.forEach(item => {
    const listItem = document.createElement('li');
    listItem.innerHTML = \`\${item.name} <button data-id="\${'}item.id\${'}">Удалить</button>\`;
...
document.getElementById('add-item').addEventListener('click', () => {
  ...
  const itemName = itemNameInput.value.trim(); // Получаем значение и убираем пробелы

  if (itemName !== '') {
    addItem(itemName);
    itemNameInput.value = ''; // Очищаем поле ввода
  }
});

// Обработчик события для кнопок "Удалить" (делегирование событий)
...
// Инициализация списка при загрузке страницы (если есть данные в localStorage, например)
renderList();
`
  }}
  options={{
