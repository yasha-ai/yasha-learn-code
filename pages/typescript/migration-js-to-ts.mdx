...
<Sandpack
  ...
  files={{
    "/index.ts": `
// index.ts
// Пример миграции JavaScript в TypeScript

// Изначальный JavaScript код
function greet(name) {
  return "Привет, " + name + "!";
}

// Добавляем типизацию TypeScript
function greetTs(name: string): string {
  return \`Привет, \${name}!\`;
}

// Функция для отображения приветствия
function displayGreeting(greetingFunction: (name: string) => string, name: string) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (greetButton && nameInput) {
    greetButton.addEventListener("click", () => {
      ...
      displayGreeting(greetTs, name); // Используем TypeScript функцию
    });
  }
});
...
// Добавляем HTML элементы динамически
...
buttonElement.id = 'greetButton';
buttonElement.textContent = 'Поздороваться';
