...
<Sandpack
      ...
      files={{
        "/index.ts": `
// index.ts
// Демонстрация strict mode и компиляторных флагов TypeScript

// 1. strictNullChecks: пример с проверкой на null
...
greetButton.addEventListener("click", () => {
  ...
  greetingDiv.textContent = greet(name || null); // Передаем null если поле пустое
});

// 2. strictFunctionTypes: пример с обработчиками событий
interface Animal {
  name: string;
}
...
const animalHandler = (animal: Animal) => {
  console.log(\`Обрабатываем животное: \${animal.name}\`);
};
...
// const dogHandler: (dog: Dog) => void = animalHandler; // Ошибка с strictFunctionTypes

animalHandler(dog); // Это безопасно, т.к. Dog является Animal

// 3. strictBindCallApply: пример с использованием call
function multiply(a: number, b: number): number {
  return a * b;
}

const result = multiply.call(null, 5, 10);
console.log(\`Результат умножения: \${result}\`);

// 4. strictPropertyInitialization: пример с классом
class Greeter {
  greeting: string;
...
<html>
<head>
  <title>TypeScript Strict Mode Demo</title>
  <style>
    body {
