...
// Интерактивный пример: Извлечение типа элемента массива
...
extractButton?.addEventListener("click", () => {
  try {
    ...
    // Функция для извлечения типа элемента массива
    function getArrayElementType(arr: any[]): string {
      type ArrayElement<T> = T extends (infer U)[] ? U : never;
      type ElementType = ArrayElement<typeof arr>;

      if (typeof parsedArray[0] === 'number') {
        return 'number';
      } else if (typeof parsedArray[0] === 'string') {
        return 'string';
      } else if (typeof parsedArray[0] === 'boolean') {
        return 'boolean';
      } else {
        return 'any';
      }
    ...
    const elementType = getArrayElementType(parsedArray);
    resultElement!.textContent = \`Тип элемента массива: \${elementType}\`;

  } catch (error) {
    resultElement!.textContent = "Ошибка: Некорректный формат массива (JSON)";
  }
});
`
    ,
...
<html>
<head>
  <title>TypeScript infer Example</title>
  <style>
    body {
...
<body>
  <h1>TypeScript infer Example</h1>

  <label for="arrayInput">Введите массив (JSON):</label>
  <input type="text" id="arrayInput" placeholder="[1, 2, 3]" value="[1, 2, 3]">

  <button id="extractButton">Извлечь тип элемента</button>
