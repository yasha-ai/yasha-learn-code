...
// Функция для создания UserPreview
function getUserPreview(user: User): UserPreview {
  return {
    id: user.id,
    name: user.name,
...
// Обновляем HTML с использованием полученных типов
const userPreview = getUserPreview(user);
const publicUser: PublicUser = { id: user.id, name: user.name, email: user.email };
const publicProduct: PublicProduct = { id: product.id, name: product.name, price: product.price, description: product.description, categoryId: product.categoryId };
const primaryColors: PrimaryColors[] = ['red', 'green', 'blue'];
...
document.body.innerHTML = \`
  <div style="padding: 20px; font-family: sans-serif; background-color: #222; color: #fff;">
    ...
    <pre>\${'{}'}
      <code>\${JSON.stringify(userPreview, null, 2)}</code>
    </pre>

    <h2>Public User (Pick)</h2>
    <pre>\${'{}'}
      <code>\${JSON.stringify(publicUser, null, 2)}</code>
    </pre>

    <h2>Public Product (Omit)</h2>
    <pre>\${'{}'}
      <code>\${JSON.stringify(publicProduct, null, 2)}</code>
    </pre>

    <h2>Primary Colors (Exclude)</h2>
    <pre>\${'{}'}
      <code>\${JSON.stringify(primaryColors, null, 2)}</code>
    </pre>

    <button id="updateButton" style="padding: 10px; background-color: #4CAF50; color: white; border: none; cursor: pointer;">Update User Name</button>
    <input type="text" id="nameInput" placeholder="Enter new name" style="padding: 10px; margin-left: 10px; color: black;" />
