...
<Sandpack
  ...
  files={{
    "/index.ts": `
// TypeScript: Вариантность типов (Variance)
...
log('<b>Ковариантность (Covariance)</b>');
...
// Можем читать как Animal
animals.forEach(animal => {
  log(\`Animal name: \${animal.name}\`); // ✓ Безопасно
});
...
const animal = produceAnimal(); // Animal (на самом деле Dog)
log(\`Produced animal sound: \${animal.makeSound()}\`);

// --------------------------------------------------
// Контравариантность (Contravariance)
// --------------------------------------------------

log('<br><b>Контравариантность (Contravariance)</b>');

type Consumer<T> = (value: T) => void;

const consumeAnimal: Consumer<Animal> = (animal) => {
  log(\`Consumed animal: \${animal.name}\`);
};
...
// Пример с Logger (по мотивам урока)
interface Logger {
  log(message: string): void;
}
...
// Контравариантность в параметрах функций (небезопасно, если strictFunctionTypes=false)
// const basicLog: LogFunction = verboseLog; // ✗ Небезопасно!
...
log('<br><b>Инвариантность (Invariance)</b>');

// Пример с изменяемым массивом (инвариантность)
class AnimalHolder {
    animal: Animal;
...
let animalHolder: AnimalHolder = new AnimalHolder(new Animal("Generic Animal"));
// Следующая строка вызовет ошибку, потому что AnimalHolder<Animal> не является подтипом или супертипом AnimalHolder<Dog>
// animalHolder = new AnimalHolder<Dog>(new Dog("Rover"));

log("Инвариантность: типы должны совпадать точно.");
`
  }}
  options={{
    showNavigator: false,
