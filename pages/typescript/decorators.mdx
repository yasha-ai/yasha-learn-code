...
// Декоратор класса для логирования создания экземпляра
function logClass(target: Function) {
  console.log(\`Класс \${target.name} создан.\`);
}

// Декоратор метода для логирования вызова метода и его результата
function logMethod(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
  const originalMethod = descriptor.value;
...
// Декоратор свойства для установки свойства только для чтения
function readonly(target: any, propertyKey: string) {
  Object.defineProperty(target, propertyKey, {
    writable: false,
  });
...
class Calculator {
  ...
  updateDisplay() {
    ...
    if (versionElement) {
      versionElement.textContent = \`Версия: \${this.version}\`;
    }
  }
}
...
document.getElementById('addButton')?.addEventListener('click', () => {
  const x = Number((document.getElementById('num1') as HTMLInputElement)?.value) || 0;
  const y = Number((document.getElementById('num2') as HTMLInputElement)?.value) || 0;
...
<html>
<head>
  <title>TypeScript Decorators Example</title>
  <meta charset="UTF-8" />
