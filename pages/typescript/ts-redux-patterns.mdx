...
export type AppDispatch = ThunkDispatch<UsersState, any, UsersActionTypes>;
...
export const removeUser = (id: string): UsersActionTypes => ({
  type: REMOVE_USER,
  payload: { id },
});

export const updateUser = (user: User): UsersActionTypes => ({
  type: UPDATE_USER,
  payload: user,
});

const fetchUsersRequest = (): UsersActionTypes => ({
  type: FETCH_USERS_REQUEST,
});

const fetchUsersSuccess = (users: User[]): UsersActionTypes => ({
  type: FETCH_USERS_SUCCESS,
  payload: users,
});

const fetchUsersFailure = (error: string): UsersActionTypes => ({
  type: FETCH_USERS_FAILURE,
  payload: error,
});

// Асинхронное действие с Redux Thunk
export const fetchUsers = () => {
  return async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      dispatch(fetchUsersSuccess(data.slice(0, 3))); // Берем только первых 3 пользователя для примера
    } catch (error: any) {
...
fetchButton.textContent = 'Загрузить пользователей';
...
const render = () => {
  ...
  userListDiv.innerHTML = \`
    ...
    \${state.error ? \`<p style="color: red;">Ошибка: \${'}state.error\${'}'}</p>\` : ''}
    <ul>
      \${state.list.map(user => \`<li key="\${'}user.id\${'}">\${'}user.name\${'} (\${'}user.email\${'})</li>\`).join('')}
    </ul>
  \`;
};

fetchButton.addEventListener('click', () => {
  store.dispatch(fetchUsers() as any); // TODO: fix type
});

appDiv?.append(fetchButton, userListDiv);
