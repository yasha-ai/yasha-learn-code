...
function validateString(str: string) {
    resultElement.innerHTML = ""; // Очищаем предыдущие результаты

    for (let name in validators) {
        let validator = validators[name];
        let isMatch = validator.isAcceptable(str);
        const resultText = document.createElement("p");
        resultText.textContent = \`'\${'}str\${'}' \${'}isMatch ? "matches" : "does not match"\${'} \${'}name\${'}'\`;
...
document.body.innerHTML = \`
  <div>
    <label for="inputString">Введите строку для валидации:</label>
    <input type="text" id="inputString" placeholder="Например, Hello или 98052">
    <button id="validateButton">Проверить</button>
    <div id="result"></div>
  </div>
\`;
`
