...
<Sandpack
  ...
  files={{
    "/index.ts": `
// TypeScript: Record и Extract

// Record<K, T>: Создает объектный тип с ключами типа K и значениями типа T.
// Extract<T, U>: Извлекает из T типы, которые можно присвоить U.
...
console.log("Роли пользователей:", roles);

// Пример использования Record с enum
enum Permission {
  Read = "read",
  Write = "write",
  Delete = "delete",
...
console.log("Права пользователей:", userPermissions);

// Пример использования Extract
type AvailableActions = "create" | "read" | "update" | "delete";
type ReadOnlyActions = "read" | "delete";

// Извлекаем только те действия, которые доступны для чтения
type ExtractedActions = Extract<AvailableActions, ReadOnlyActions>;

const readOnlyActions: ExtractedActions[] = ["read", "delete"];

console.log("Действия только для чтения:", readOnlyActions);

// Интерактивный пример с кнопками и отображением результата
...
const recordExample = document.createElement("div");
recordExample.innerHTML = \`
  <h3>Пример Record</h3>
  <p>Введите роль пользователя:</p>
  <input type="text" id="roleInput" placeholder="Например, guest" style="padding: 5px; margin-right: 10px; color: black;" />
