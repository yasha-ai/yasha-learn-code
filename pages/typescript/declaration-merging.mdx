...
button1.textContent = 'Показать User';
button1.addEventListener('click', () => {
  displayResult(\`User: \${JSON.stringify(user)}\`);
});

const button2 = document.createElement('button');
button2.textContent = 'Log Message';
button2.addEventListener('click', () => {
  Utils.log('Button clicked!');
  displayResult('Logged a message.');
});

const button3 = document.createElement('button');
button3.textContent = 'Increment Counter';
button3.addEventListener('click', () => {
  const count = counter();
  displayResult(\`Counter: \${count}\`);
});
