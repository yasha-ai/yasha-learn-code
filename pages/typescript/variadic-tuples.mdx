...
// Variadic Tuple Types пример

// Функция, которая принимает строку и произвольное количество чисел
function logWithNumbers(message: string, ...numbers: number[]): void {
  const outputElement = document.getElementById('output');
  if (!outputElement) return;

  outputElement.innerHTML += \`<div>\${message}: \${numbers.join(', ')}<\/div>\`;
}

// Функция для добавления префикса к массиву
function addPrefix<T extends unknown[], Prefix>(prefix: Prefix, ...arr: T): [Prefix, ...T] {
  return [prefix, ...arr];
}

// Пример использования addPrefix
const prefixedArray = addPrefix("ID:", 1, 2, 3);
console.log("Prefixed Array:", prefixedArray);

// Функция для демонстрации variadic tuple types
function demonstrateVariadicTuples() {
  const outputElement = document.getElementById('output');
  if (!outputElement) return;

  outputElement.innerHTML = ''; // Очищаем предыдущий вывод

  logWithNumbers("Numbers:", 1, 2, 3, 4, 5);

  // Пример с addPrefix
  const prefixed = addPrefix("Log:", true, "hello", 42);
  outputElement.innerHTML += \`<div>Prefixed: \${JSON.stringify(prefixed)}<\/div>\`;
}

// Создаем кнопку для запуска демонстрации
const button = document.createElement('button');
button.textContent = 'Demonstrate Variadic Tuples';
button.onclick = demonstrateVariadicTuples;
document.body.appendChild(button);

// Создаем div для вывода
