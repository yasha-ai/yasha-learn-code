...
<Sandpack
  ...
  files={{
    "/index.ts": `
// Модификаторы Mapped Types: readonly, ?, +, -
...
// readonlyUser.age = 35; // Ошибка: свойство 'age' доступно только для чтения

const mutableUser: Mutable<Readonly<User>> = {
  id: "3",
  name: "Peter Pan",
  age: 10,
...
function displayUserInfo(user: User, elementId: string) {
...
displayUserInfo(user, "user-info");
displayUserInfo(mutableUser, "mutable-user-info");
displayUserInfo(partialUser as User, "partial-user-info"); // Приведение типов, т.к. Partial<User> может не содержать всех полей
displayUserInfo(requiredUser, "required-user-info");

// Интерактивный элемент: кнопка для изменения возраста пользователя
const changeAgeButton = document.getElementById("change-age-button");
if (changeAgeButton) {
  changeAgeButton.addEventListener("click", () => {
    mutableUser.age = mutableUser.age + 1;
    displayUserInfo(mutableUser, "mutable-user-info");
  });
}
...
style.textContent = \`
  ...
  #user-info, #mutable-user-info, #partial-user-info, #required-user-info {
...
<html>
<head>
  <title>TypeScript Mapped Types Modifiers</title>
  <meta charset="UTF-8" />
