...
<Sandpack
  template="vanilla-ts"
  files={{
    "/index.ts": `import { proxy, useSnapshot } from 'valtio';
import React from 'react';
...
// 2. Создаем прокси-объект состояния
const counterState = proxy<CounterState>({
  count: 0,
  lastUpdated: Date.now(),
  increment: () => {
    counterState.count++;
    counterState.lastUpdated = Date.now();
  },
  decrement: () => {
    counterState.count--;
    counterState.lastUpdated = Date.now();
...
const Counter: React.FC = () => {
  // 3. Используем useSnapshot для получения снимка состояния
  const snapshot = useSnapshot(counterState);
  ...
  return (
    <div style={{
      ...
      <p>Счетчик: {snapshot.count}</p>
      <p>Последнее обновление: {new Date(snapshot.lastUpdated).toLocaleTimeString()}</p>
      <button onClick={counterState.increment} style={{
        padding: '10px 20px',
        fontSize: '16px',
        margin: '10px',
      ...
      <button onClick={counterState.decrement} style={{
        padding: '10px 20px',
        fontSize: '16px',
        margin: '10px',
