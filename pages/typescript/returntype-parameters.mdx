...
<Sandpack
  ...
  files={{
    "/index.ts": `
// ReturnType и Parameters в TypeScript

// ReturnType: извлекает тип возврата функции
type ReturnType<T extends (...args: any) => any> = T extends (...args: any) => infer R ? R : any;

// Parameters: извлекает типы параметров функции как tuple
...
type GreetingReturnType = ReturnType<typeof createGreeting>; // string
...
// Практический пример: Type-safe декоратор
type AnyFunction = (...args: any[]) => any;
...
button.addEventListener('click', () => {
  ...
  const result = loggedMultiply(num1, num2);
