...
![Иллюстрация к уроку](/lessons/typescript-basic-types.png)
...
let numbers: number[] = [1, 2, 3, 4, 5];
let names: string[] = ["Alice", "Bob", "Charlie"];
let booleanValues: Array<boolean> = [true, false, true];
...
if (app) {
    app.innerHTML = \`
        <div style="padding: 20px; font-family: sans-serif; background-color: #282c34; color: #fff; border-radius: 5px;">
            <h1>TypeScript: Базовые типы</h1>
            <p>Возраст: \${age}</p>
            <p>Цена: \${price}</p>
            <p>Имя: \${name}</p>
            <p>Сообщение: \${message}</p>
            <p>Взрослый: \${isAdult}</p>
            <p>Залогинен: \${isLoggedIn}</p>
            <p>Текст из unknown: \${text}</p>
            <button id="updateButton">Изменить имя</button>
        </div>
    \`;
