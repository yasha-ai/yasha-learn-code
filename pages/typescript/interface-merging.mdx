...
// Слияние интерфейсов для Config
interface Config {
  apiUrl: string;
}
...
function displayInfo() {
  ...
  if (outputElement) {
    outputElement.innerHTML = \`
      <p>User Details: \${'user.getDetails()'}</p>
...
// Слияние интерфейсов для EventListener
...
// Пример реализации EventListener (необязательно)
...
// Кнопка для запуска отображения информации
