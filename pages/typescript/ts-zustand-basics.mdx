...
// --- Как использовать это хранилище в React-компоненте (для демонстрации) ---
// В данном примере мы используем ванильный JavaScript для демонстрации,
// но Zustand обычно используется с React.
...
counterElement.appendChild(document.createTextNode('Count: '));
const countSpan = document.createElement('span');
counterElement.appendChild(countSpan);
counterElement.appendChild(document.createElement('br'));
...
counterElement.appendChild(document.createElement('br'));
counterElement.appendChild(document.createTextNode('Amount: '));
counterElement.appendChild(amountInput);
...
incrementButton.addEventListener('click', () => {
  ...
  useCounterStore.getState().increment(amount);
});

decrementButton.addEventListener('click', () => {
  const amount = parseInt(amountInput.value, 10) || 1;
  useCounterStore.getState().decrement(amount);
});
