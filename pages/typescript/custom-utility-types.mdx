...
const App = () => {
  ...
  const handlePortChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPort(isNaN(value) ? undefined : value);
  };

  // Пример использования ConfigWithRequiredHost
  const config: ConfigWithRequiredHost = {
    host: host, // host теперь обязателен
    port: port, // port опционален
  };

  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#282c34', color: 'white' }}>
      <h1>Custom Utility Types Demo</h1>
      <div>
        <label htmlFor="host">Host (required):</label>
        <input
          type="text"
          id="host"
          value={host}
          onChange={handleHostChange}
          style={{ margin: '5px', padding: '5px', color: 'black' }}
        />
