...
// Функции-конструкторы для создания branded типов
function createUserId(id: string): UserId {
  if (!id.startsWith('user_')) {
    throw new Error('Invalid user ID format');
  }
...
function getUser(id: UserId): User | null {
  ...
  if (id === createUserId('user_123')) {
    return { id: id, name: 'Alice', email: createEmail('alice@example.com') };
  }
  return null;
}

function getPost(id: PostId): Post | null {
  // Имитация получения поста из базы данных
  if (id === createPostId('post_456')) {
    return { id: id, title: 'My Post', content: 'This is the content of my post.' };
  }
  return null;
}
...
if (user) {
  displayOutput(\`User: \${'}user.name'} (\${'}user.email'}) with ID: \${'}user.id'}\`);
} else {
  displayOutput('User not found.');
}

if (post) {
  displayOutput(\`Post: \${'}post.title'} with ID: \${'}post.id'}\`);
} else {
  displayOutput('Post not found.');
}

try {
  createUserId(invalidUserId);
} catch (error: any) {
  displayOutput(\`Error creating user ID: \${'}error.message'}\`);
}

// Пример попытки передачи PostId в функцию getUser (вызовет ошибку компиляции, если раскомментировать)
// getUser(postId); // Ошибка типов!
...
try {
  const positiveNumber = positive(5);
  displayOutput(\`Positive number: \${'}positiveNumber'}\`);
} catch (error: any) {
  displayOutput(\`Error creating positive number: \${'}error.message'}\`);
}
...
<html>
<head>
  <title>TypeScript Branded Types Example</title>
  <style>
    body {
...
<body>
  <h1>TypeScript Branded Types Example</h1>
  <div id="output"></div>
