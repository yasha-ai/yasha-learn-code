...
// Пример с User-Defined Type Guards
interface User {
  type: 'user';
  name: string;
  email: string;
...
// Type guard function для определения Admin
function isAdmin(person: Person): person is Admin {
  return person.type === 'admin';
}

// Type guard function для определения User
...
function processPerson(person: Person) {
  if (isAdmin(person)) {
    // TypeScript знает, что person это Admin
    return \`Permissions: \${person.permissions.join(', ')}\`;
  } else {
    // TypeScript знает, что person это User
...
function processString(value: unknown) {
  try {
    assertIsString(value);
    return \`Upper case: \${value.toUpperCase()}\`;
  } catch (error: any) {
...
const stringInput = document.getElementById('stringInput') as HTMLInputElement;
const stringProcessButton = document.getElementById('stringProcessButton') as HTMLButtonElement;
const animalTypeInput = document.getElementById('animalTypeInput') as HTMLInputElement;
const animalNameInput = document.getElementById('animalNameInput') as HTMLInputElement;
const animalSoundButton = document.getElementById('animalSoundButton') as HTMLButtonElement;
const outputDiv = document.getElementById('output') as HTMLDivElement;

userButton.addEventListener('click', () => {
  const user: Person = { type: 'user', name: 'John', email: 'john@example.com' };
  outputDiv.textContent = processPerson(user);
});

adminButton.addEventListener('click', () => {
  const admin: Person = { type: 'admin', name: 'Jane', permissions: ['read', 'write'] };
  outputDiv.textContent = processPerson(admin);
});

stringProcessButton.addEventListener('click', () => {
  const inputValue = stringInput.value;
  outputDiv.textContent = processString(inputValue);
});
...
animalSoundButton.addEventListener('click', () => {
  ...
  let animal: Animal;
  if (animalType === 'dog') {
    animal = { name: animalName, bark: () => 'Woof!' };
  } else {
    animal = { name: animalName, meow: () => 'Meow!' };
  }

  outputDiv.textContent = animalSound(animal);
});
`
  }}
  options={{
