...
export default function App() {
  ...
  const [numberBoxValue, setNumberBoxValue] = useState(0);
  const [stringBoxValue, setStringBoxValue] = useState("");
  ...
  const numberBox = new Box<number>(numberBoxValue);
  const stringBox = new Box<string>(stringBoxValue);

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInputValue(e.target.value);
  };

  const handleNumberBoxChange = () => {
    setNumberBoxValue(Number(inputValue));
  };

  const handleStringBoxChange = () => {
    setStringBoxValue(inputValue);
  };

  return (
    <div style={{ padding: "20px", fontFamily: "sans-serif", backgroundColor: "#282c34", color: "white" }}>
      <h1>Generic Classes Example</h1>
      <div>
