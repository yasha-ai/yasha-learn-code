...
<Sandpack
  ...
  files={{
    "/index.ts": `
// Демонстрация Generics в TypeScript

// 1. Функция с Generic типом
function getFirstElement<T>(arr: T[]): T | undefined {
  return arr[0];
}
...
const numberArray = [1, 2, 3];
const firstNumber = getFirstElement(numberArray); // firstNumber имеет тип number
console.log("Первый элемент массива чисел:", firstNumber);

const stringArray = ["a", "b", "c"];
const firstString = getFirstElement(stringArray); // firstString имеет тип string
console.log("Первый элемент массива строк:", firstString);

// 2. Интерфейс с Generic типом
interface Result<T> {
  success: boolean;
  data?: T;
  error?: string;
...
const result = processData("Long Data");
if (result.success) {
  console.log("Результат обработки данных:", result.data);
} else {
  console.error("Ошибка обработки данных:", result.error);
}

// 3. Класс с Generic типом
class DataHolder<T> {
  data: T;
...
const numberHolder = new DataHolder<number>(10);
console.log("Число в DataHolder:", numberHolder.getData());

const stringHolder = new DataHolder<string>("Hello");
console.log("Строка в DataHolder:", stringHolder.getData());

// Интерактивный пример:
// Создадим элемент input и кнопку, чтобы пользователь мог ввести данные,
// и мы будем использовать DataHolder для хранения этих данных.
...
button.addEventListener("click", () => {
  ...
  const dataHolder = new DataHolder<string>(inputValue);
  output.textContent = "Сохраненные данные: " + dataHolder.getData();
});
...
`
  ...
  options={{
    ...
    editorHeight: 400,
    preview: {
      head: '<style>body { background-color: #282c34; color: white; padding: 20px; font-family: sans-serif; }</style>'
    }
  }}
/>
