...
// 1. Generic function с inference
...
// 2. Generic function с явным указанием типа
function createArray<T>(length: number, value: T): T[] {
  return Array(length).fill(value);
}

const numberArray = createArray<number>(3, 10);
const stringArray = createArray<string>(2, "world");
...
const stringHolder = new DataHolder<string>("initial value");
...
const updateOutput = () => {
    outputDiv.innerHTML = \`
        <p>identity(123): \${numberResult}</p>
        <p>identity("hello"): \${stringResult}</p>
        <p>createArray<number>(3, 10): [\${numberArray.join(', ')}]</p>
        <p>createArray<string>(2, "world"): [\${stringArray.join(', ')}]</p>
        <p>KeyValuePair<string, number>: key=\${pair1.key}, value=\${pair1.value}</p>
        <p>KeyValuePair<number, string>: key=\${pair2.key}, value=\${pair2.value}</p>
        <p>DataHolder<number>: \${numberHolder.getData()}</p>
        <p>DataHolder<string>: \${stringHolder.getData()}</p>
    \`;
};

updateOutput();

// Input and button for DataHolder<string>
const input = document.createElement('input');
input.type = 'text';
input.value = stringHolder.getData();
document.body.appendChild(input);

const button = document.createElement('button');
button.textContent = 'Update String Data';
button.style.marginLeft = '10px';
button.onclick = () => {
    stringHolder.setData(input.value);
    updateOutput();
};
